...
$info = array();
add_to_breadcrumbs(array('link' => BASEDIR.'blog.php',
					   'title' => $locale['global_081b'])); // blog needs to be localised
if (isset($_GET['readmore']) && isnum($_GET['readmore'])) {
	...
	if (dbrows($result) > 0) {
		...
		$blog_info = array("blog_id" => $data['blog_id'],
			...
			"blog_author_link" => array('link' => BASEDIR."blog.php?author=".$data['user_id'],
				'name' => sprintf($locale['global_071b'], ucwords($data['user_name']))),);
		// get UF and merge for testimonial
		if ($data['user_email'] && $data['user_hide_email'] == 0) $blog_info['user_email'] = "<a href='mailto:".$data['user_email']."'>".$data['user_email']."</a>";
		if ($data['user_location']) $blog_info['user_location'] = $data['user_location'];
		if ($data['user_web']) {
			$data['user_web'] = str_replace('http://', '', $data['user_web']);
			$data['user_web'] = str_replace('https://', '', $data['user_web']);
			$blog_info['user_web'] = "<a href='http://".$data['user_web']."'>".$data['user_web']."</a>\n";
		}
		...
		add_to_breadcrumbs(array('link' => BASEDIR."blog.php?readmore=".$data['blog_id'],
							   'title' => $data['blog_subject']));
	...
	$info['allowed_filters'] = array('recent' => $locale['global_086b'],
		'comment' => $locale['global_087b'],
		'rating' => $locale['global_088b']);
	...
	if (isset($_GET['author']) && isnum($_GET['author'])) {
		$rows = dbcount("(blog_id)", DB_BLOG, "".groupaccess('blog_visibility')." AND (blog_start='0'||blog_start<=".time().") AND (blog_end='0'||blog_end>=".time().") AND blog_draft='0' AND blog_name='".$_GET['author']."'");
		if ($rows) {
			// apply filter.
	...
	// Category Page
		...
		if (dbrows($result)) {
			...
			$info['blog_cat_image'] = $data['blog_cat_image'] && file_exists(IMAGES_BC.$data['blog_cat_image']) ? "<img class='img-responsive' src='".IMAGES_BC.$data['blog_cat_image']."' />" : "<img class='img-responsive' src='holder.js/80x80/text:".$locale['no_image']."/grey' />";
			...
			if ($rows) {
				...
				ORDER BY blog_sticky DESC, ".$cat_filter." LIMIT ".$_GET['rowstart'].",".$settings['blogperpage']);
				$info['blog_item_rows'] = $rows;
				add_to_breadcrumbs(array('link' => BASEDIR."blog.php?cat_id=".$data['blog_cat_id'],
									   'title' => $data['blog_cat_name']));
			}
			...
			if ($rows) {
				...
				ORDER BY blog_sticky DESC, ".$cat_filter." LIMIT ".$_GET['rowstart'].",".$settings['blogperpage']);
				$info['blog_item_rows'] = $rows;
				add_to_breadcrumbs(array('link' => BASEDIR."blog.php?cat_id=".$_GET['cat_id'],
	...
	if (!empty($info['blog_item_rows'])) {
		while ($data = dbarray($result)) {
			...
			$blog_info[$i] = array("blog_id" => $data['blog_id'],
		...
		if (isset($_GET['author']) && isnum($_GET['author']) && isset($blog_info[1])) add_to_breadcrumbs(array('link' => BASEDIR."blog.php?author=".$blog_info[1]['user_id'], 'title' =>$locale['global_070'].ucwords($blog_info[1]['user_name'])));
