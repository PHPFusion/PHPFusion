...
class Shoutbox {
    ...
    private function deleteShout($id) {
        if (!empty($id)) {
            ...
            add_notice('success', self::$locale['SB_shout_deleted']);
        }
        defined('ADMIN_PANEL') ?
            redirect(clean_request("section=shoutbox", ["", "aid"])) :
    ...
    private function getSecureShoutId($shout_hash) {
        ...
        if (!empty($decrypted_hash)) {
            ...
            if (count($hash) === 3) {
                [$shout_token_id, $user_token_id, $time_token] = $hash;
    ...
    private function setShoutboxDb() {
        if (check_post('shout_box')) {
            ...
            if (!flood_control("shout_datestamp", DB_SHOUTBOX, "shout_name='".$this->data['shout_name']."'")) {
                if (fusion_safe()) {
                    ...
                    add_notice("success", empty($this->data['shout_id']) ? self::$locale['SB_shout_added'] : self::$locale['SB_shout_updated']);
                }
            } else {
        ...
        if (check_post('sb_settings')) {
            ...
            if (fusion_safe()) {
                ...
                add_notice("success", self::$locale['SB_update_ok']);
        ...
        if (check_post('sb_delete_old') && check_post('num_days') && isnum(post('num_days'))) {
            ...
            add_notice("warning", number_format($numrows) . " / " . post('num_days') . self::$locale['SB_delete_old']);
            defined('ADMIN_PANEL') ?
                redirect(clean_request("section=shoutbox", ["", "aid"])) :
                redirect($this->postLink);
        }
