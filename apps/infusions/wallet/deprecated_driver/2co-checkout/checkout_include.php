...
echo "<input type='hidden' name='order_total' value='".$cart_total."' />\n";
			...
			if (cccheck) {
				tokenRequest();
					...
					sleep(2000).then(() => {
						...
						$.ajax({
							type:'POST',
							url:'".INFUSIONS."wallet/checkout/checkout_proccess.php',
							dataType: 'json',
							cache: false,
							data:formData,
							success:function(data){
								if(data.status == '1'){
								if ($cart_total == data.total) {
										...
										$('#payment_results').html('There was a problem verifying the cost, please contact support. Be aware that any maniupulation atempts are subject to prosecution.');
									}
								} else {
