...
+--------------------------------------------------------+
| Filename: viewthread_options.php
...
if (isset($_POST['delete_posts']) && isset($_POST['delete_post']) && is_array($_POST['delete_post']) && count($_POST['delete_post'])) {
    ...
    if (!$thread_count) {
        redirect(BASEDIR."forum/viewforum.php?forum_id=".$fdata['forum_id']);
    }
} else if (isset($_POST['move_posts']) && isset($_POST['delete_post']) && is_array($_POST['delete_post']) && count($_POST['delete_post'])) {
    $move_posts = "";
    $array_post = [];
    $f_post = FALSE;
    $dell_f_post = FALSE;
        ...
        if (dbrows($move_result)) {
            ...
            if (!isset($_POST['new_forum_id']) && !$f_post_blo) {
                ...
                if (dbrows($fl_result) > 0) {
                    ...
                    echo "<a href='".BASEDIR."forum/viewthread.php?thread_id=".$pdata['thread_id']."&amp;rowstart=".$_GET['rowstart']."'>".$locale['609']."</a><br /><br />\n";
                    echo "</div></div>";
                }
            } else if (isset($_POST['new_forum_id']) && isnum($_POST['new_forum_id']) && !isset($_POST['new_thread_id']) && !$f_post_blo) {
                $forum_list = "";
                $tl_result = dbquery("
            ...
            } else if (isset($_GET['sv']) && isset($_POST['new_forum_id']) && isnum($_POST['new_forum_id']) && isset($_POST['new_thread_id']) && isnum($_POST['new_thread_id'])) {
                $move_posts_add = "";
                if (!dbcount("(thread_id)", DB_THREADS, "thread_id='".$_POST['new_thread_id']."' AND forum_id='".$_POST['new_forum_id']."'")) {
                    redirect(BASEDIR."forum/viewthread.php?thread_id=".$fdata['thread_id']."&amp;rowstart=".$_GET['rowstart']."&amp;error=1");
                }
    ...
    if ($message != "") {
        ...
        echo "<a href='".BASEDIR."forum/viewthread.php?thread_id=".$fdata['thread_id']."&amp;rowstart=".$_GET['rowstart']."'>".$locale['609']."</a><br />";
        echo "</div></div>\n";
        closetable();
        require_once THEMES."templates/footer.php";
        die();
