...
// user fields
$user_field = ["user_sig" => FALSE, "user_web" => FALSE];
if (iMEMBER) {
...
if (($rows > $posts_per_page) || ($can_post || $can_reply)) {
    ...
    if ($rows > $posts_per_page) {
        echo "<td style='padding:4px 0px 4px 0px'>".makepagenav($_GET['rowstart'], $posts_per_page, $rows, 3, BASEDIR."forum/viewthread.php?thread_id=".$_GET['thread_id']."&amp;")."</td>\n";
    }
    if (iMEMBER && $can_post) {
...
if ($rows != 0) {
    ...
    if ($poll_on_first_page_only && $poll_there && $poll_data) {
        $i = 1;
        if ($can_vote) {
            echo "<form name='voteform' method='post' action='".BASEDIR."forum/viewthread.php?forum_id=".$fdata['forum_id']."&amp;thread_id=".$_GET['thread_id']."'>\n";
        }
    ...
    if (iMOD) {
        echo "<form name='mod_form' method='post' action='".BASEDIR."forum/viewthread.php?thread_id=".$_GET['thread_id']."&amp;rowstart=".$_GET['rowstart']."'>\n";
    }
