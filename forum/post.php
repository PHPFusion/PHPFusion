...
if (isset($_GET['action']) && ($_GET['action'] == 'voteup' or $_GET['action'] == 'votedown') && ($info['forum_vote'] != 0 && checkgroup($info['forum_vote'])) && isset($_GET['thread_id']) && isnum($_GET['thread_id']) && isset($_GET['post_id']) && isnum($_GET['post_id'])) {
	...
	if (!$res) {
		...
		$self_post = dbcount("('post_id')", DB_POSTS, "post_id='".$_GET['post_id']."' AND post_user='".$userdata['user_id']."");
		if (!$self_post) {
			...
			if ($info['forum_answer_threshold'] >0) {
				...
				if ($info['forum_answer_threshold'] !=0 && $v_data['vote_points'] >= $info['forum_answer_threshold']) {
					$result = dbquery("UPDATE ".DB_THREADS." SET 'thread_locked'='1' WHERE thread_id='".$v_data['thread_id']."'");
				}
			}
...
elseif ((isset($_GET['action']) && $_GET['action'] == 'newthread') && ($info['forum_post'] != 0 && checkgroup($info['forum_post']))) {
	...
	$result = dbquery("SELECT * FROM ".DB_THREADS." WHERE thread_id='".$_GET['thread_id']."' AND forum_id='".$info['forum_id']."' AND thread_hidden='0'");
	if (dbrows($result)) {
		$data = dbarray($result);
	...
	// fetch data.
	$verify_thread = dbcount("('thread_id')", DB_THREADS, "thread_id='".$_GET['thread_id']."' AND forum_id='".$info['forum_id']."' AND thread_hidden='0'");
	if ($verify_thread) {
