        ...
        if ($tdata['thread_poll'] == 1 && ($pdata['post_author'] == $tdata['thread_author']) && ($userdata['user_id'] == $tdata['thread_author'] || iSUPERADMIN || iMOD)) {
            ...
            if (isset($_POST['poll_options']) && is_array($_POST['poll_options'])) {
                ...
                foreach ($_POST['poll_options'] as $poll_option) {
                    ...
                    } else if (isset($_POST['add_poll_option'][$i])) {
                        if (trim($poll_option)) {
                    ...
                    } else if (isset($_POST['update_poll_option'][$i])) {
                        if (trim($poll_option)) {
                            $poll_opts[] = trim(stripinput($poll_option));
...
if (isset($_POST['savechanges'])) {
            ...
            if ($message != "") {
                if (isset($_POST['hide_edit'])) {
                    ...
                    if ($thread_lastpost['post_id'] == $_GET['post_id'] && time() - $pdata['post_datestamp'] < 5 * 60) {
                        $post_edit_time = 0;
                        $reason = "";
                    } else if ($settings['forum_editpost_to_lastpost']) {
                        $post_edit_time = time();
                        ...
                        if ($attach['name'] != "" && !empty($attach['name']) && is_uploaded_file($attach['tmp_name'])) {
                            ...
                            if (preg_match("/^[-0-9A-Z_\[\]]+$/i", $attachname) && $attach['size'] <= $settings['attachmax']) {
                                ...
                                if (in_array($attachext, $attachtypes)) {
                                    ...
                                    if (!$error)
                                        $result = dbquery("INSERT INTO ".DB_FORUM_ATTACHMENTS." (thread_id, post_id, attach_name, attach_ext, attach_size) VALUES ('".$_GET['thread_id']."', '".$_GET['post_id']."', '".$attachname."', '".$attachext."', '".$attach['size']."')");
                                } else {
    ...
    if (iMOD || iADMIN)
        echo "<label><input type='checkbox' name='hide_edit' value='1' /> ".$locale['487']."</label><br />\n";
    if (iMOD || iADMIN)
        echo "<label><input type='checkbox' name='post_locked' value='1'".$post_locked." /> ".$locale['488']."</label><br />\n";
