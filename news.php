...
if (!isset($_GET['readmore']) || !isnum($_GET['readmore'])) {
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			$news_cat_image = "<a href='".($settings['news_image_link'] == 0 ? "".BASEDIR."news_cats.php?cat_id=".$data['news_cat'] : BASEDIR."news.php?readmore=".$data['news_id'] )."'>";
	...
	if (dbrows($result)) {
		...
		if ($data['news_image_t1'] && $settings['news_image_readmore'] == "0") {
			...
			$news_cat_image = "<a href=\"javascript:;\" onclick=\"window.open('".$settings['siteurl']."/images/news/".$data['news_image']."','','scrollbars=yes,toolbar=no,status=no,resizable=yes,width=".($img_size[0]+20).",height=".($img_size[1]+20)."')\"><img src='".IMAGES_N_T.$data['news_image_t1']."' alt='".$data['news_subject']."' class='img-responsive news-category' /></a>";
		} elseif ($data['news_cat_image']) {
		...
		if ($pagecount > 1) {
			echo "<div align='center' style='margin-top:5px;'>\n".makepagenav($_GET['rowstart'], 1, $pagecount, 3, BASEDIR."news.php?readmore=".$_GET['readmore']."&amp;")."\n</div>\n";
		}
		if ($data['news_allow_comments']) { showcomments("N", DB_NEWS, "news_id", $_GET['readmore'], BASEDIR."news.php?readmore=".$_GET['readmore']); }
		if ($data['news_allow_ratings']) { showratings("N", $_GET['readmore'], BASEDIR."news.php?readmore=".$_GET['readmore']); }
	} else {
