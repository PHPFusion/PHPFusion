...
if (count(fusion_get_enabled_languages()) > 1) {
	...
	foreach(fusion_get_enabled_languages() as $languages) {
		$language_opts .= "<li class='display-block text-left'><a class='display-block' style='width:100%' href='".clean_request('lang='.$languages, array('lang'), false, '&amp;')."'><img alt='".$languages."' class='m-r-5' src='".BASEDIR."locale/$languages/$languages-s.png'> $languages</a></li>\n";
	}
	$language_opts .= "</ul>\n";
	$language_opts .= "</li>\n";
}
