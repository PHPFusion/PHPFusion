        ...
        if (count($global_comments['data']) > 0) {
            foreach ($global_comments['data'] as $i => $comment_data) {
                if (isset($comments_type[$comment_data['comment_type']]) && isset($link_type[$comment_data['comment_type']])) {
                    ...
                    $html .= "<div id='comment_action-$i' class='btn-group pull-right display-none' style='position:absolute; right: 30px; margin-top:25px;'>
        ...
        foreach ($global_submissions['data'] as $i => $submit_date) {
            if (isset($submit_data[$submit_date['submit_type']])) {
                ...
                if (!empty($review_link)) {
                    $html .= '<a class="btn btn-xs btn-default m-l-10 pull-right display-none" id="submission_action-'.$i.'" href="'.$review_link.'">'.$locale['286'].'</a>';
                }
                $html .= '</div>';
            }
        }
...
function render_admin_icon() {
    ...
    if (count($admin_icons['data']) > 0) {
        foreach ($admin_icons['data'] as $i => $data) {
            ...
            $html .= "<div class='overflow-hide'><a class='icon-title' href='".$data['admin_link'].$aidlink."'>".$data['admin_title']."</a></div>";
            $html .= "</div>";
            $html .= "</div>";
        }
    }
