...
$document_language = function_exists('get_inline_language') ? get_inline_language() : str_replace('_', '-', $locale['xml_lang']);

require_once INCLUDES.'frameworks/framework_engine.php';
fusion_framework_boot('site');

//if (defined('FUSION_DEVELOPMENT')) :
//$vite_connection = @fsockopen("127.0.0.1", 5173, $errno, $errstr, 0.1);
//if ($vite_connection) {
//    $vite_running = true;
//    fclose($vite_connection);
//} else {
//    // We don't care about the error, so we just set this to false
//    $vite_running = false;
//}
//endif;
...
echo "<!DOCTYPE html>";
echo "<html lang='" . htmlspecialchars($document_language, ENT_QUOTES, 'UTF-8') . "' dir='" . $locale['text-direction'] . "'" . ( $settings['create_og_tags'] ? " prefix='og: http://ogp.me/ns#'" : "" ) . ">";
echo "<head>";
...
fusion_apply_hook('fusion_framework_header', 'site');
...
fusion_apply_hook('fusion_framework_footer', 'site');
...
$fusion_javascript = OutputHandler::$javascript;
if ( ! empty($fusion_javascript) ) {
    echo "<script>{$fusion_javascript}</script>";
}
