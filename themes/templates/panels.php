...
if (checkgroup($panel_data['panel_access']) && in_array(LANGUAGE, $p_langs))  { $panels_cache[$panel_data['panel_side']][] = $panel_data; }
 } else {
		...
		if (!defined("ADMIN_PANEL")) {
			if (check_panel_status($p_side['side'])) {
				foreach ($panels_cache[$p_key + 1] as $p_data) {
					...
					{
						if (($p_data['panel_side'] != 2 && $p_data['panel_side'] != 3 && $p_data['panel_side'] != 5 && $p_data['panel_side'] != 6)
