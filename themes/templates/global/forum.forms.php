...
function postform($data) {
	...
	if ($data['edit'] or $data['reply'])  {
		if ($data['reply']) {
			...
			echo $data['first_post'] == $_GET['post_id'] ?
				form_text($locale['forum_0600'], 'thread_subject', 'thread_subject', $data['thread_subject'], array('required' => 1, 'placeholder'=>$locale['forum_2001'], 'error_text' => '', 'class'=>'m-t-20 m-b-20')) :
				"<h4 class='m-b-20'>".$locale['forum_2002'].$data['thread_subject']."</h4>\n ".form_hidden('', 'thread_subject', 'thread_subject', $data['thread_subject']);
		}
	} else {
		echo form_text($locale['forum_0600'], 'thread_subject', 'thread_subject', $data['thread_subject'], array('required' => 1, 'placeholder'=>$locale['forum_2001'], 'error_text' => '', 'class'=>'m-t-20 m-b-20'));
	}
	echo form_textarea($locale['forum_0601'], 'post_message', 'post_message', $data['post_message'], array('required' => 1, 'error_text' => '', 'autosize'=>1, 'no_resize'=>1, 'preview'=>1, 'form_name'=>'input_form', 'bbcode' => 1));
	echo $data['edit'] ? form_text($locale['forum_0611'], 'post_editreason', 'post_editreason', $data['post_editreason'], array('placeholder'=>'Edit reasons', 'error_text' => '', 'class'=>'m-t-20 m-b-20')) : '';
	...
	if ($info['forum_poll'] && checkgroup($info['forum_poll']) && ($data['edit'] or $data['new'])) {
		...
		echo form_text($locale['forum_0604'], 'forum_poll_title', 'forum_poll_title', $data['forum_poll_title'], array('max_length' => 255, 'placeholder'=>'Enter a Poll Title', 'inline'=>1));
		...
		if (isset($data['poll_opts']) && !empty($data['poll_opts'])) {
			foreach ($data['poll_opts'] as $poll_option) {
				...
				echo form_text($locale['forum_0605'].' '.$i, "poll_options[$i]", "poll_options[$i]", $poll_option, array('max_length' => 255, 'placeholder'=>'Poll Options', 'inline'=>1, 'class'=>'m-b-0'));
				if ($data['edit']) {
				...
				if ($i == count($data['poll_opts'])) {
					if ($data['edit']) {
						$i++;
						echo form_text($locale['forum_0605'].' '.$i, "poll_options[$i]", "poll_options[$i]", '', array('max_length'=>255, 'placeholder'=>'Poll Options', 'inline'=>1, 'class'=>'m-b-0'));
					}
					echo "<div class='col-xs-12 col-sm-offset-3 m-b-10'>\n";
					echo form_button($locale['forum_0608'], 'add_poll_option', 'add_poll_option', $locale['forum_0608'], array('class' => 'btn-default btn-sm m-r-10', 'icon'=>'entypo plus-circled'));
					if ($data['edit']) {
			...
			echo form_text($locale['forum_0606'], "poll_options[1]", "poll_options[1]", '', array('max_length' => 255, 'placeholder'=>'Poll Options', 'inline'=>1));
			echo form_text($locale['forum_0607'], "poll_options[2]", "poll_options[2]", '', array('max_length' => 255, 'placeholder'=>'Poll Options', 'inline'=>1));
...
if (!function_exists('post_preview')) {
	function post_preview($data) {
		...
		if ($can_poll && !empty($data['forum_poll_title']) && !empty($data['poll_opts'])) {
			...
			echo form_button($locale['forum_2010'], 'vote', 'vote', 'vote', array('class'=>'btn btn-sm btn-primary m-l-20 ', 'deactivate'=>1));
			echo "</div>\n";
			echo "</div>\n";
		}
		echo "</div>\n";
