...
if (isset($_POST['save'])) {
    ...
    if (isset($_POST['delete']) && (isset($_POST['poll_id']) && isnum($_POST['poll_id']))) {
        $result = dbcount("(poll_id)", DB_POLLS, "poll_id='".$_POST['poll_id']."'");
        if (!empty($result)) {
            $result = dbquery("DELETE FROM ".DB_POLLS." WHERE poll_id='".$_POST['poll_id']."'");
        }
...
$result = dbquery("SELECT poll_id, poll_title, poll_language
        FROM ".DB_POLLS." ".(multilang_table("PO") ? "WHERE poll_language='".LANGUAGE."'" : "")." ORDER BY poll_id DESC");
if (dbrows($result) > 0) {
    ...
    echo openform('editform', 'post', FUSION_SELF.$aidlink);
    echo form_select('poll_id', '', '', array(
        'options' => $editlist,
...
if (multilang_table("PO")) {
    ...
    echo "<td class='tbl'>\n";
    echo form_select('poll_language', '', $poll_language, array(
