...
if (iMEMBER && $article_settings['article_allow_submission']) {
	...
	if (isset($_GET['submitted']) && $_GET['submitted'] == "a") {
		...
		if (isset($_POST['preview_article'])) {
			...
			$article_snippet = stripinput($_POST['article_article']);
			$article_article = stripinput($_POST['article_article']);
			opentable($article_subject);
			echo $locale['478b']." ".nl2br(parseubb(html_entity_decode(stripslashes($article_snippet))))."<br /><br />";
			echo $locale['472b']." ".nl2br(parseubb(html_entity_decode(stripslashes($article_body))));
			closetable();
		}
		...
		echo form_select('article_keywords', $locale['articles_0204'], $criteriaArray['article_keywords'], array(
			"max_length" => 320, "inline" => TRUE, "placeholder" => $locale['articles_0204a'], "width" => "100%",
			"error_text" => $locale['articles_0204a'], "tags" => TRUE, "multiple" => TRUE
		));

		echo form_select_tree("article_cat", $locale['articles_0201'], $criteriaArray['article_cat'], array(
			"width" => "250px", "inline" => TRUE, "no_root" => true,
		...
		echo form_textarea('article_article', $locale['articles_0203'], $criteriaArray['article_article'], array(
			"required" => $article_settings['article_extended_required'] ? TRUE : FALSE, "html" => TRUE,
