...
require_once INFUSIONS.'rss_feeds_panel/RSS.php';
	 ...
	 $result = dbquery("SELECT f.forum_id, f.forum_name, f.forum_lastpost, f.forum_postcount,
        ...
        LEFT JOIN ".DB_POSTS." p ON t.thread_id = p.post_id
    ...
    if (dbrows($result) != 0) {
        while ($data = dbarray($result)) {
            $rss->AddItem($data['thread_subject'].' ['.$data['forum_name'].']', $settings['siteurl'].'forum/viewthread.php?forum_id='.$data['forum_id'].'&thread_id='.$data['thread_id'], $data['post_message']);
        }
    } else {
