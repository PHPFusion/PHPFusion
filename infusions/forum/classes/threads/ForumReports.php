        ...
        if (iMEMBER) {
            ...
            if (isset($_POST['submit_report'])) {
                ...
                if (isset($selected_comments[$report_reason])) {
                    ...
                    fusion_stop();
                    add_notice("danger", "Your report selection could not be determined. Please try again.");
                    redirect($exit_uri);
                }
                ...
                if (isset($_GET['rtid']) && isnum($_GET['rtid']) && $post_id = dbcount("(post_id)", DB_FORUM_POSTS, "post_id=:pid", [":pid" => intval($_GET['rtid'])])) {
                    ...
                    if (dbcount("(report_id)", DB_FORUM_REPORTS, "report_user=:uid AND post_id=:rtid AND report_status=0", [':uid' => $userdata['user_id'], ":rtid" => intval($_GET['rtid'])])) {
                        fusion_stop();
                        add_notice("danger", "There are open reports that you  made on this post earlier. Please wait for moderator to review the post.");
                        redirect($exit_uri);
                    }
                    $rdata = [
                        "report_id"        => 0,
                    ...
                    if (fusion_safe()) {
                        ...
                        add_notice("success", "Your feedback has been received. Thank you.");
                        redirect($exit_uri);
                    }
                }
