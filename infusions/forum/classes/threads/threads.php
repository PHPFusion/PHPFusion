    ...
    public function set_threadInfo() {
        ...
        if (!empty($this->thread_data) && !empty($_GET['thread_id']) && isnum($_GET['thread_id']) && $this->check_forum_access($forum_index, 0, $_GET['thread_id'])) {
            ...
            if (isset($_GET['action'])) {
                ...
                if ($this->thread_info['thread']['forum_users'] == true) {
                    $this->thread_info['thread_users'] = $this->get_participated_users($this->thread_info);
                }

                self::check_download_request();
                // +1 threadviews
                $this->increment_thread_views($this->thread_info['thread']['thread_id']);
