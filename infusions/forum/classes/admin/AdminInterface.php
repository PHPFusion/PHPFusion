...
abstract class AdminInterface extends ForumServer {
    ...
    private static function setLocale() {
        self::$locale = fusion_get_locale( "", [ FORUM_ADMIN_LOCALE, SETTINGS_LOCALE, FORUM_TAGS_LOCALE, FORUM_RANKS_LOCALE ] );
     ...
     * @return mixed
     */
    protected function checkForumName( $forum_name, $forum_id = 0 ) {
        if ( $forum_name ) {
            $cond = "forum_name='".$forum_name."'".( multilang_table( 'FO' ) ? " AND ".in_group( 'forum_language', LANGUAGE ) : '' );
            if ( $forum_id ) {
                $cond = "forum_name='".$forum_name."' AND forum_id !='".$forum_id."'".( multilang_table( 'FO' ) ? " AND ".in_group( 'forum_language', LANGUAGE ) : '' );
            }
