...
class Admin {
    ...
    private function display_forum_move_form() {
        ...
        echo form_button('forum_remove', $locale['forum_049'], 'forum_remove', array(
	...
	public function display_forum_form() {
        ...
        echo form_text('forum_name', $locale['forum_006'], $this->data['forum_name'], array(
            ...
            form_textarea('forum_description', $locale['forum_007'], $this->data['forum_description'], array(
                'autosize' => 1,
                'bbcode' => 1,
                'form_name' => 'inputform'
            )).
		...
		echo form_select_tree('forum_cat', $locale['forum_008'], $this->data['forum_cat'], array(
                'add_parent_opts' => 1,
                'disable_opts' => $self_id,
                'hide_disabled' => 1
            ), DB_FORUMS, 'forum_name', 'forum_id', 'forum_cat', $self_id).

            form_select('forum_type', $locale['forum_009'], $this->data['forum_type'], array("options" => $type_opts)).

		    form_select('forum_language', $locale['forum_010'], $this->data['forum_language'], array("options" => $language_opts)).

		    form_text('forum_order', $locale['forum_043'], $this->data['forum_order'], array('number' => 1)).

		    form_button('save_forum', $this->data['forum_id'] ? $locale['forum_000a'] : $locale['forum_000'], $locale['forum_000'], array('class' => 'btn btn-sm btn-success'));
		...
		echo form_select('forum_meta', $locale['forum_012'], $this->data['forum_meta'], array(
			'tags' => 1,
			'multiple' => 1,
			'width' => '100%'
		));
		if ($this->data['forum_image'] && file_exists(FORUM."images/".$this->data['forum_image'])) {
			...
			echo form_select('forum_image_header', $locale['forum_056'], '', array(
				'inline' => TRUE,
				'options' => $header_opts
			));
			echo form_text('forum_image_url', $locale['forum_014'], '', array(
		...
		echo form_textarea('forum_rules', $locale['forum_017'], $this->data['forum_rules'], array(
			'autosize' => 1,
			'bbcode' => 1
		));
		echo "</div><div class='col-xs-12 col-sm-4 col-md-4 col-lg-4'>\n";
        ...
        openside('');
		echo form_checkbox('forum_lock', $locale['forum_026'], $this->data['forum_lock'], array(
                "reverse_label" => true,
            )).
            form_checkbox('forum_users', $locale['forum_024'], $this->data['forum_users'], array(
                "reverse_label" => true,
            )).
		    form_checkbox('forum_quick_edit', $locale['forum_021'], $this->data['forum_quick_edit'], array(
                "reverse_label" => true,
            )).
		    form_checkbox('forum_merge', $locale['forum_019'], $this->data['forum_merge'], array(
                "reverse_label" => true,
            )).
		    form_checkbox('forum_allow_attach', $locale['forum_020'], $this->data['forum_allow_attach'], array(
                "reverse_label" => true,
            )).
		    form_checkbox('forum_allow_poll', $locale['forum_022'], $this->data['forum_allow_poll'], array(
                "reverse_label" => true,
            )).
	...
	private function display_forum_permissions_form() {
		...
		echo form_select('forum_access', $locale['forum_031'], $data['forum_access'], array(
			'inline' => TRUE,
		...
		echo form_select('forum_answer_threshold', $locale['forum_040'], $data['forum_answer_threshold'], array(
			'options' => $selection,
			'inline' => TRUE
		));
		closeside();
		...
		echo form_select('forum_attach', $locale['forum_034'], $data['forum_attach'], array(
			'options' => $access_opts,
			'inline' => TRUE
		));
		echo form_select('forum_attach_download', $locale['forum_035'], $data['forum_attach_download'], array(
			'options' => $public_access_opts,
			'inline' => TRUE
		));
		closeside();
		...
		echo form_select("forum_mods[]", $locale['forum_desc_003'], $data['forum_mods'], array(
			"multiple" => TRUE,
			"width" => "100%",
			"options" => $options,
			"delimiter" => ".",
    ...
    private function quick_create_forum() {
        ...
        echo form_text('forum_name', $locale['forum_006'], '', array(
            'required' => 1,
            'inline' => TRUE,
