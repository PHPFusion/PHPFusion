...
if (iMEMBER && PHPFusion\Forums\Functions::verify_forum($_GET['forum_id'])) {
	...
	if ($permission['can_post'] && $permission['can_access']) {
		...
		$info = array(
			...
			'description' => '',
			'openform' =>  openform('input_form', 'post', $form_action, array('enctype' => $permission['can_upload_attach'])), // use new permission to toggle enctype
			'closeform' => closeform(),
			'forum_id_field' => '',
			'thread_id_field' => '',
			...
			'attachment_field' => $forum->getForumPermission("can_upload_attach") ?
