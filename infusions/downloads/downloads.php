...
if (isset($_GET['download_id'])) {
    if (validate_download($_GET['download_id'])) {
        ...
        if ($info['download_rows'] > 0) {
            ...
            if ($data['download_homepage']) {
                $urlprefix = (!strstr($data['download_homepage'], "http://") && !strstr($data['download_homepage'],
                                                                                        "https://")) ? 'http://' : '';
    ...
    if (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) {
        ...
        $info['download_max_rows'] = dbcount("('download_id')", DB_DOWNLOADS,
                                             "download_cat='".intval($_GET['cat_id'])."' AND ".groupaccess('download_visibility'));
