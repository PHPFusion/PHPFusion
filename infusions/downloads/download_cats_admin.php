...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	if ($_GET['section'] == 'dadd') {
		...
		$array = array('1' => $locale['download_0303'],
		...
		openside('');
		echo form_text('cat_name', $locale['download_0300'], $cat_name, array('required' => 1,
			'error_text' => $locale['download_0351']));
		echo form_textarea('cat_description', $locale['download_0301'], $cat_description, array('resize' => 0,
			'autosize' => 1));
		echo "<div class='clearfix'>\n";
		echo form_select('cat_sort_by', $locale['download_0302'], $cat_sort_by, array('options' => $array,
			'placeholder' => $locale['choose'],
			'class' => 'pull-left m-r-10',
			'width' => '200px'));
		echo form_select('cat_sort_order', '', $cat_sort_order, array('options' => $array2,
			'placeholder' => $locale['choose'],
			'class' => 'pull-left',
			'width' => '200px'));
		echo "</div>\n";
		...
		openside('');
		echo form_select_tree("cat_parent", $locale['download_0308'], $cat_parent, array("disable_opts" => $cat_hidden,
			"hide_disabled" => 1,
			'width' => '100%'), DB_DOWNLOAD_CATS, "download_cat_name", "download_cat_id", "download_cat_parent");
		if (multilang_table("DL")) {
			echo form_select('cat_language', $locale['global_ML100'], $cat_language, array('options' => $language_opts,
				'placeholder' => $locale['choose'],
				'width' => '100%'));
		} else {
		...
		closeside();
		echo "</div>\n</div>\n";
		echo form_button('save_cat', $locale['download_0309'], $locale['download_0309'], array('class' => 'btn-success btn-sm m-t-10',
