...
class Model
{
	...
	public function readmore_Model() {
		global $locale;
		...
		$result = dbquery("
					SELECT tn.*, tc.*, tu.user_id, tu.user_name, tu.user_status, tu.user_avatar , tu.user_level, tu.user_joined,
					...
					AND news_id='".$this->news_id."' AND news_draft='0'
					LIMIT 1
					");
		if (dbrows($result)>0) {
			...
			$news_info = array(
				...
				'news_news' => $news_news[$this->rowstart],
				"news_ext" => "n",
			...
			if (fusion_get_settings("create_og_tags")) {
				...
				add_to_head("<meta property='og:url' content='".fusion_get_settings("siteurl")."infusions/news.php?readmore=$this->news_id' />");
				if ($data['news_image']) {
					$og_image = IMAGES_N.$data['news_image'];
				} else {
					$og_image = IMAGES_NC.$data['cat_image'];
				}
				$og_image = str_replace(BASEDIR, fusion_get_settings("siteurl"), $og_image);
	...
	public function general_Model() {
		...
		$info = array(
			"news_cat_id" => 0,
			"news_cat_name" => $locale['global_082'],
			"news_cat_image" => "",
			...
			if (dbrows($result)) {
				...
				if ($rows) {
					...
					$result = dbquery("
						...
						GROUP BY news_id
						ORDER BY news_sticky DESC, ".$cat_filter." LIMIT ".$this->rowstart.",".$news_settings['news_pagination']
					);
...
// Controller sanitize inputs and picks the model.
class Controller extends Model {
	public function __construct() {
		...
		if (!isset($_GET['rowstart']) || !isnum($_GET['rowstart'])) {
			...
			$this->rowstart = $_GET['rowstart'];
		}

		if (isset($_GET['readmore']) && isnum($_GET['readmore'])) {
			$this->news_id = $_GET['readmore'];
			$this->modelType = "readmore";
			$this->readmore_Model();
		} else {
...
class View
{
	...
	public function __construct($controller) {
		$this->rowstart = $controller->rowstart;
		$this->modelType = $controller->modelType;
	...
	public function output(){
		...
		// 7 backwards comp.
		$_GET['rowstart'] = $this->rowstart;
		$news_settings = get_settings("news");

		if ($this->modelType == "general") {
			render_main_news($this->info);
		} elseif ($this->modelType == "readmore") {
			render_news($this->info['news_item']);
		}
	}
}
