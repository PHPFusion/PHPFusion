...
if (iMEMBER && $blog_settings['blog_allow_submission']) {
	$criteriaArray = array("blog_subject" => "",
		"blog_cat" => 0,
		"blog_snippet" => "",
		"blog_body" => "",
	...
	if (isset($_POST['submit_blog'])) {
		$submit_info['blog_blog'] = nl2br(parseubb(stripinput($_POST['blog_blog'])));
		$submit_info['blog_body'] = nl2br(parseubb(stripinput($_POST['blog_body'])));
		$criteriaArray = array("blog_subject" => form_sanitizer($_POST['blog_subject'], "", "blog_subject"),
			"blog_cat" => form_sanitizer($_POST['blog_cat'], "", "blog_cat"),
			"blog_snippet" => form_sanitizer($submit_info['blog_blog'], "", "blog_blog"),
			"blog_body" => form_sanitizer($submit_info['blog_body'], "", "blog_body"),
		...
		if (defender::safe()) {
			$inputArray = array("submit_type" => "b",
	...
	if (isset($_GET['submitted']) && $_GET['submitted'] == "b") {
		...
		echo "<p><a href='submit.php?stype=n'>".$locale['blog_0702']."</a></p>";
		...
		if (isset($_POST['preview_blog'])) {
			...
			$blog_snippet = stripinput($_POST['blog_blog']);
		...
		echo form_text('blog_subject', $locale['blog_0422'], $criteriaArray['blog_subject'], array("required" => TRUE,
			"inline" => TRUE));
		if (multilang_table("BL")) {
			echo form_select('blog_language', $locale['global_ML100'], $criteriaArray['blog_language'], array("options" => fusion_get_enabled_languages(),
		...
		echo form_select('blog_keywords', $locale['blog_0443'], $criteriaArray['blog_keywords'], array("max_length" => 320,
											"inline" => TRUE,
											...
											"tags" => TRUE,
											"multiple" => TRUE));
		echo form_select_tree("blog_cat", $locale['blog_0423'], $criteriaArray['blog_cat'], array("width" => "250px",
											"inline" => TRUE,
											"parent_value" => $locale['blog_0424'],
											"query" => (multilang_table("BL") ? "WHERE blog_cat_language='".LANGUAGE."'" : "")), DB_BLOG_CATS, "blog_cat_name", "blog_cat_id", "blog_cat_parent");
		if ($blog_settings['blog_allow_submission_files']) {
			...
			echo form_select('blog_ialign', $locale['blog_0442'], $criteriaArray['blog_ialign'], array("options" => $alignOptions,
				"inline" => TRUE));
		}
		echo form_textarea('blog_blog', $locale['blog_0425'], $criteriaArray['blog_snippet'], array("required" => TRUE,
										  "html" => TRUE,
										  "form_name" => "submit_form",
										  "autosize" => fusion_get_settings("tinymce_enabled") ? FALSE : TRUE));
		echo form_textarea('blog_body', $locale['blog_0426'], $criteriaArray['blog_body'], array("required" => $blog_settings['blog_extended_required'] ? TRUE : FALSE,
										  "html" => TRUE,
										  "form_name" => "submit_form",
										  "autosize" => fusion_get_settings("tinymce_enabled") ? FALSE : TRUE));
