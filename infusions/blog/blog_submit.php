...
if (iMEMBER && $blog_settings['blog_allow_submission']) {
	$criteriaArray = array(
		"blog_subject" => "", "blog_cat" => 0, "blog_snippet" => "", "blog_body" => "", "blog_language" => LANGUAGE,
		"blog_keywords" => "", "blog_ialign" => "",
	);
	if (isset($_POST['submit_blog'])) {
		...
		if (defender::safe()) {
			$inputArray = array(
				"submit_type" => "b", "submit_user" => $userdata['user_id'], "submit_datestamp" => time(),
	...
	if (isset($_GET['submitted']) && $_GET['submitted'] == "b") {
		...
		if (multilang_table("BL")) {
			echo form_select('blog_language', $locale['global_ML100'], $criteriaArray['blog_language'], array(
				"options" => fusion_get_enabled_languages(), "placeholder" => $locale['choose'], "width" => "250px",
				"inline" => TRUE,
			));
		} else {
		...
		echo form_select('blog_keywords', $locale['blog_0443'], $criteriaArray['blog_keywords'], array(
			"max_length" => 320, "inline" => TRUE, "placeholder" => $locale['blog_0444'], "width" => "100%",
		...
		echo form_select_tree("blog_cat", $locale['blog_0423'], $criteriaArray['blog_cat'], array(
			"width" => "250px", "inline" => TRUE, "parent_value" => $locale['blog_0424'],
		...
		if ($blog_settings['blog_allow_submission_files']) {
			$file_input_options = array(
				...
				'max_height' => $blog_settings['blog_photo_max_h'], 'max_byte' => $blog_settings['blog_photo_max_b'],
				// set thumbnail
				'thumbnail' => 1, 'thumbnail_w' => $blog_settings['blog_thumb_w'],
				'thumbnail_h' => $blog_settings['blog_thumb_h'], 'thumbnail_folder' => 'thumbs', 'delete_original' => 0,
				// set thumbnail 2 settings
				'thumbnail2' => 1, 'thumbnail2_w' => $blog_settings['blog_photo_w'],
				'thumbnail2_h' => $blog_settings['blog_photo_h'], 'type' => 'image', "inline" => TRUE,
			);
		...
		echo form_textarea('blog_body', $locale['blog_0426'], $criteriaArray['blog_body'], array(
			"required" => $blog_settings['blog_extended_required'] ? TRUE : FALSE, "html" => TRUE,
			"form_name" => "submit_form", "autosize" => fusion_get_settings("tinymce_enabled") ? FALSE : TRUE
		));
