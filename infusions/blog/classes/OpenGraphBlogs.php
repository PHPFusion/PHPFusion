...
class OpenGraphBlogs extends OpenGraph {
    public static function ogBlog($blog_id = 0) {
        ...
        $result = dbquery("SELECT blog_subject, blog_blog, blog_keywords, blog_image_t1, blog_image_t2 FROM ".DB_BLOG." WHERE blog_id = :blogid", [':blogid' => $blog_id]);
        if (dbrows($result)) {
            $data = dbarray($result);
            ...
            } else if (!empty($data['blog_image_t2']) && file_exists(IMAGES_B_T.$data['blog_image_t2'])) {
