...
class WeblinksAdmin extends WeblinksAdminModel {
    ...
    private function weblinkContent_form() {
        ...
        if (!fusion_get_settings("tinymce_enabled")) {
            ...
            $ExtendedSettings = ["required" => ($this->weblinksSettings['links_extended_required'] ? TRUE : FALSE), "type" => "tinymce", "tinymce" => "advanced", "error_text" => $this->locale['WLS_0270']];
        }
    ...
    private function display_weblinkButtons($unique_id, $breaker = TRUE) {
        ...
        echo form_button("cancel", $this->locale['cancel'], $this->locale['cancel'], ["class" => "btn-default m-r-10", "icon" => "fa fa-fw fa-times", "input-id" => "cancel-".$unique_id.""]);
        echo form_button("save", $this->locale['save'], $this->locale['save'], ["class" => "btn-success m-r-10", "icon" => "fa fa-fw fa-hdd-o", "input-id" => "save-".$unique_id.""]);
        echo form_button("save_and_close", $this->locale['save_and_close'], $this->locale['save_and_close'], ["class" => "btn-primary m-r-10", "icon" => "fa fa-fw fa-floppy-o", "input-id" => "save_and_close-".$unique_id.""]);
        echo "</div>\n";
        if ($breaker) {
            echo "<hr />\n";
        }
            ...
            <div id="weblink_filter_options"<?php echo($filter_empty ? " style='display: none;'" : ""); ?>>
                <div class="display-inline-block">
                    ...
                    echo form_select("weblink_status", "", $filter_values['weblink_status'], [
                        "allowclear" => TRUE, "placeholder" => "- ".$this->locale['WLS_0123']." -", "options" => [0 => $this->locale['WLS_0124'], 1 => $this->locale['draft']]
                    ]);
                    ?>
                </div>
                <div class="display-inline-block">
        ...
        <div class="table-responsive">
            <table id="links-table" class="table table-striped">
                ...
                <?php if (dbrows($result2) > 0) :
                    while ($data = dbarray($result2)) : ?>
                        ...
                        <tr id="link-<?php echo $data['weblink_id']; ?>" data-id="<?php echo $data['weblink_id']; ?>">
                            <td><?php echo form_checkbox("weblink_id[]", "", "", ["value" => $data['weblink_id'], "class" => "m-0", 'input_id' => 'link-id-'.$data['weblink_id']]) ?></td>
                            ...
                            <td>
                                ...
                                <a href="<?php echo $delete_link; ?>" title="<?php echo $this->locale['delete']; ?>"
                                   onclick="return confirm('<?php echo $this->locale['WLS_0111']; ?>')"><?php echo $this->locale['delete']; ?></a>
                            </td>
                        </tr>
                        <?php
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7"
                            class="text-center"><?php echo($weblink_cats ? ($filter_empty ? $this->locale['WLS_0112'] : $this->locale['WLS_0113']) : $this->locale['WLS_0114']); ?></td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
