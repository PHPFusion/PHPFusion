...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
    $result = dbcount("(weblink_cat)", DB_WEBLINKS, "weblink_cat='".$_GET['cat_id']."'") || dbcount("(weblink_cat_id)", DB_WEBLINK_CATS,
                                                                                                    "weblink_cat_parent='".$_GET['cat_id']."'");
    if (!empty($result)) {
    ...
    if (isset($_POST['save_cat'])) {
        ...
        if (isnum($data['cat_sort_by']) && $data['cat_sort_by'] == "1") {
            ...
            if (isnum($_POST['cat_sort_by']) && $data['cat_sort_by'] == "2") {
                $data['weblink_cat_sorting'] = "weblink_name ".($data['cat_sort_order'] == "ASC" ? "ASC" : "DESC");
            } else {
                if (isnum($_POST['cat_sort_by']) && $data['cat_sort_by'] == "3") {
