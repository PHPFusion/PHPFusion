...
class ViewThread extends ForumServer {
     ...
     * @param $thread_id
     */
    private function addViewCount(int $thread_id) {
        $days_to_keep_session = 7;
        $time = session_get(['thread', $thread_id]);
        if (!$time) {
            session_add(['thread', $thread_id], TIME);
            dbquery("UPDATE ".DB_FORUM_THREADS." SET thread_views=thread_views+1 WHERE thread_id=:tid", [':tid' => (int)$thread_id]);
        } else {
            if ($time <= TIME - ($days_to_keep_session * 3600 * 24)) {
                session_remove(['thread', $thread_id]);
            }
        }
    }
