...
class Forum_Viewer {
    ...
    public static function getInstance() {
        if (self::$instance == NULL) {
            ...
            add_to_head("<link rel='stylesheet' type='text/css' href='".INFUSIONS."forum/templates/forum.min.css'>");
        }
    ...
    private function forum_index(array $info = [], $id = 0) {
        ...
        if (!empty($info['forums'][$id])) {
            ...
            foreach ($info['forums'][$id] as $forum_id => $data) {
                ...
                if ($data['forum_type'] == 1) {
                        ...
                        foreach ($data['child'] as $child_id => $child_data) {
                            ...
                            if (isset($child_data['child'])) {
                                ...
                                foreach ($child_data['child'] as $sub_forum_id => $sub_forum_data) {
                                    ...
                                    $sub_forum_image = ($sub_forum_data['forum_image'] ?
                                        thumbnail($sub_forum_data['forum_image'], '10px') :
                                        "<div class='img-circle m-t-5 m-b-10' style='background:#$sub_forum_color; color:#$font_color;padding: 0px 5px;font-size:12px;margin-right:10px;'>".$sub_forum_data['forum_icon']."</div>");
                    ...
                    $html->set_block('forum_content', array(
        ...
        $html->set_tag('forum_panel', ['forum_panel' => $this->sticky_discussions_panel()]);
    ...
    private function forum_report($info) {
        //print_p($info);
        $locale = fusion_get_locale();
        $html = \PHPFusion\Template::getInstance('forum_reports');
    ...
    private function forum_viewforum($info) {
        ...
        $html->set_tag('baselink', $info['link']);
        $html->set_tag('description', $info['forum_description']);
        if ($info['forum_rules']) {
    ...
    public function forum_threads_item($info) {
        ...
        $html = \PHPFusion\Template::getInstance('forum_thread');
        $html->set_locale($locale);
        $html->set_template($file_path);
