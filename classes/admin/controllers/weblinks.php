...
class WeblinksAdmin extends WeblinksAdminModel {
    ...
    private function execute_Update() {
        ...
        if (post('save') or post('save_and_close')) {
            ...
            if (fusion_safe()) {
                ...
                $update_datestamp = post('update_datestamp');
                // Update
    ...
    private function display_weblinks_listing() {
        ...
        if (post('table_action') && isset($allowed_actions[$_POST['table_action']])) {
            ...
            if (empty($input) && post('table_action') == "verify") {
                self::verifyLink();
        ...
        if (post('p-submit-weblink_name')) {
            $search_string['weblink_name'] = [
                'input' => sanitizer('weblink_name', '', 'weblink_name'), 'operator' => "LIKE", 'option' => "AND"
            ];
            $search_string['weblink_url'] = [
                'input' => sanitizer('weblink_name', '', 'weblink_name'), 'operator' => "LIKE", 'option' => "OR"
            ];
        ...
        if ($weblink_status) {
            $search_string['weblink_status'] = [
                'input' => sanitizer('weblink_status', '', 'weblink_status'), 'operator' => "=", 'option' => "AND"
            ];
        }

        //$weblink_visibility = filter_input(INPUT_POST, 'weblink_visibility', FILTER_DEFAULT);
        if (post('weblink_visibility')) {
        ...
        //$default_display = 16;
        $limit = 16;
        $postdisplay = post('weblink_display');
        $getdisplay = get('weblink_display');
        $limits = (!empty($postdisplay) ? $postdisplay : (!empty($getdisplay) ? $getdisplay : $limit));
        if (!empty($limits)) {
            $limit = $limits;
        }
        ...
        $result2 = dbquery("SELECT  w.*, wc.*
            ...
            WHERE ".(multilang_table("WL") ? in_group('weblink_language', LANGUAGE) : "")."
            $sql_condition
        ...
        // Filters
        $filter_values = [
            'weblink_name'       => !empty($weblink_name) ? sanitizer('weblink_name', '', 'weblink_name') : '',
            'weblink_status'     => sanitizer('weblink_status', '', 'weblink_status'),
            'weblink_cat'        => !empty($weblink_cat) ? sanitizer('weblink_cat', '', 'weblink_cat') : '',
            'weblink_visibility' => sanitizer('weblink_visibility', '', 'weblink_visibility')
        ];
        ...
        <div class="display-block">
            <div class="display-inline-block m-l-10">
                ...
                echo form_select('weblink_display', $this->locale['WLS_0132'], $limit, [
                    'width'   => '100px',
                    'options' => [1 => 1, 5 => 5, 10 => 10, 16 => 16, 25 => 25, 50 => 50, 100 => 100]
                ]);
                ?>
            </div>
    ...
    private function execute_Delete() {
        ...
        if ($action && ($action == "delete") && $weblink_id) {
