...
class FaqAdmin extends FaqAdminModel {
        ...
        if (isset($_POST['save_cat'])) {
            ...
            if (fusion_safe()) {
                if ($this->cat_data['faq_cat_id']) {
                    ...
                    add_notice('success', $this->locale['faq_0040']);
                } else {
                    if (!dbcount("(faq_cat_id)", DB_FAQ_CATS, "faq_cat_name=:faq_cat_name", [':faq_cat_name' => $this->cat_data['faq_cat_name']])) {
                        ...
                        add_notice('success', $this->locale['faq_0039']);
                    } else {
                        \Defender::stop();
                        \Defender::inputHasError('faq_cat_name');
                        add_notice('warning', $this->locale['faq_0042']);
                    }
                }
    ...
    private function execute_Delete() {
        if (isset($_GET['action']) && $_GET['action'] == "delete" && isset($_GET['faq_id']) && isnum($_GET['faq_id'])) {
            ...
            if (dbcount("(faq_id)", DB_FAQS, "faq_id=:faqid", [':faqid' => $faq_id]) && !dbcount("(faq_id)", DB_FAQS, "faq_cat_id=:faqcatid", [':faqcatid' => $faq_id])) {
                ...
                add_notice('success', $this->locale['faq_0032']);
            } else {
                add_notice('warning', $this->locale['faq_0035']);
                add_notice('warning', $this->locale['faq_0036']);
            }
    ...
    private function execute_Update() {
        if ((isset($_POST['save'])) or (isset($_POST['save_and_close']))) {
            ...
            if (!$this->faq_data['faq_order']) {
                $this->faq_data['faq_order'] = dbresult(dbquery("SELECT MAX(faq_order) FROM ".DB_FAQS." ".(multilang_table("FQ") ? "WHERE ".in_group('faq_language', LANGUAGE)." AND" : "WHERE")." faq_cat_id='".$this->faq_data['faq_cat_id']."'"), 0) + 1;
            }
            ...
            if (fusion_safe()) {
                ...
                if (dbcount("(faq_id)", DB_FAQS, "faq_id='".$this->faq_data['faq_id']."'")) {
                    ...
                    add_notice('success', $this->locale['faq_0031']);
                    ...
                    add_notice('success', $this->locale['faq_0030']);
                }
    ...
    private function display_faq_listing() {
        ...
        if (isset($_POST['table_action']) && isset($allowed_actions[$_POST['table_action']])) {
            ...
            add_notice('warning', $this->locale['faq_0034']);
            redirect(FUSION_REQUEST);
        }
        ...
        if (isset($_POST['delete_faq_cat']) && isset($_POST['faq_cat_id']) && isnum($_POST['faq_cat_id'])) {
            ...
            add_notice('success', $this->locale['faq_0041']);
            redirect(FUSION_SELF.fusion_get_aidlink());
        }
        ...
        add_to_jquery("
            ...
            // Select Change
            $('#faq_status, #faq_visibility, #faq_name, #faq_display, #faq_cat').bind('change', function(e){
