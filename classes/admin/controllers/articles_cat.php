...
class ArticlesCategoryAdmin extends ArticlesAdminModel {
    ...
    private function display_article_cat_form() {
        ...
        if ((isset($_POST['save_cat'])) || (isset($_POST['save_cat_and_close']))) {
            ...
            $categoryNameCheck = [
                "when_updating" => "article_cat_name='".$inputArray['article_cat_name']."' AND article_cat_id !='".$inputArray['article_cat_id']."' ".(multilang_table("AR") ? "AND ".in_group('article_cat_language', LANGUAGE) : ""),
        ...
        if (isset($_POST['table_action']) && isset($allowed_actions[$_POST['table_action']])) {
            ...
            if (!empty($input)) {
                ...
                foreach ($input as $article_cat_id) {
                    // check input table
                    if (dbcount("('article_cat_id')", DB_ARTICLE_CATS, "article_cat_id=:articlecat", [':articlecat' => intval($article_cat_id)]) && fusion_safe()) {
