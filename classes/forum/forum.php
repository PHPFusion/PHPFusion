...
class Forum extends Forum_Server {
    ...
    /**
     * The forum threads where viewforum requests are available
     *
     * @return array
     */
    private function get_view_forum_threads() {
        $userdata = fusion_get_userdata();
        $locale = fusion_get_locale();
        $forum_settings = self::get_forum_settings();
        ...
        if (dbrows($result)) {
            ...
            $this->forum_info['forum_moderators'] = Forum_Moderator::parse_forum_mods($this->forum_info['forum_mods']);
            Forum_Moderator::define_forum_mods($this->forum_info);
