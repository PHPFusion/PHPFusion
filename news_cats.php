...
if (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) {
    ...
    if (dbrows($result) || $_GET['cat_id'] == 0) {
        ...
        if ($rows) {
            ...
            $result2 = dbquery("SELECT news_id, news_subject FROM ".DB_NEWS." ".(multilang_table("NS") ? "WHERE news_language='".LANGUAGE."' AND" : "WHERE")." news_cat='".$_GET['cat_id']."' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".time().") AND (news_end='0'||news_end>=".time().") AND news_draft='0' ORDER BY news_datestamp DESC");
    ...
    if (dbrows($result)) {
        ...
        while ($data = dbarray($result)) {
            ...
            if ($rows) {
                $result2 = dbquery("SELECT news_id, news_subject FROM ".DB_NEWS." ".(multilang_table("NS") ? "WHERE news_language='".LANGUAGE."' AND" : "WHERE")." news_cat='".$data['news_cat_id']."' AND ".groupaccess('news_visibility')." AND (news_start='0'||news_start<=".time().") AND (news_end='0'||news_end>=".time().") AND news_draft='0' ORDER BY news_datestamp DESC LIMIT 10");
                while ($data2 = dbarray($result2)) {
                    echo THEME_BULLET." <a href='news.php?readmore=".$data2['news_id']."'>".$data2['news_subject']."</a><br />\n";
                }
                if ($rows > 10) {
                    echo "<div style='text-align:right'>".THEME_BULLET." <a href='".BASEDIR."news_cats.php?cat_id=".$data['news_cat_id']."'>".$locale['405']."</a></div>\n";
                }
            } else {
    ...
    if (dbrows($result)) {
        ...
        $res = 1;
        if ($nrows > 10) {
            echo "<div style='text-align:right'>".THEME_BULLET." <a href='".BASEDIR."news_cats.php?cat_id=0'>".$locale['405']."</a></div>\n";
        }
        echo "</td>\n</tr>\n";
    }
    if ($res == 1) {
