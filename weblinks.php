...
if (!isset($_GET['cat_id']) || !isnum($_GET['cat_id'])) {
	...
	count(weblink_id) AS weblink_count
	FROM ".DB_WEBLINK_CATS." wc
	LEFT JOIN ".DB_WEBLINKS." w ON (w.weblink_cat=wc.weblink_cat_id)
	".(multilang_table("WL") ? "WHERE weblink_cat_language='".LANGUAGE."' AND" : "WHERE")."
	".groupaccess('weblink_cat_access')."
	...
	$info = array();
	$result = dbquery("SELECT weblink_cat_name, weblink_cat_sorting, weblink_cat_access FROM
	".DB_WEBLINK_CATS." ".(multilang_table("WL") ? "WHERE weblink_cat_language='".LANGUAGE."' AND" : "WHERE")."
	".groupaccess('weblink_cat_access')." AND weblink_cat_id='".$_GET['cat_id']."'");
	...
	if (dbrows($result) != 0) {
		...
		$max_rows = dbcount("(weblink_id)", DB_WEBLINKS, "weblink_cat='".$_GET['cat_id']."'");
