...
if (!isset($_GET['cat_id']) || !isnum($_GET['cat_id'])) {
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td valign='top'><a href='".BASEDIR."faq.php?cat_id=".$data['faq_cat_id']."'>".$data['faq_cat_name']."</a> <span class='small2'>($num)</span>\n";
	...
	if ($data = dbarray(dbquery("SELECT faq_cat_name,faq_cat_language FROM ".DB_FAQ_CATS." ".(multilang_table("FQ") ? "WHERE faq_cat_language='".LANGUAGE."' AND" : "WHERE")." faq_cat_id='".$_GET['cat_id']."'"))) {
		...
		echo "<td class='tbl2'>\n<a href='".BASEDIR."faq.php'>".$locale['400']."</a> &gt;";
		echo "<a href='".BASEDIR."faq.php?cat_id=".$_GET['cat_id']."'>".$data['faq_cat_name']."</a></td>\n";
		echo "</tr>\n</table>\n";
