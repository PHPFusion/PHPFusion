...
if (isset($_GET['article_id'])&& isnum($_GET['article_id'])){
	...
	if (dbrows($result)) {
		$data = dbarray($result);
		$str .= "<a href='".BASEDIR."articles.php'><strong>".$locale['404']."</strong></a>";
		$str .= " &raquo; <a href='".BASEDIR."articles.php?cat_id=".$data['article_cat']."'>".$data['article_cat_name']."</a>";
		$str .= " &raquo; <a href='".BASEDIR."articles.php?article_id=".$_GET['article_id']."'>".$data['article_subject']."</a>";
		$isTrue = true;
	}
	...
	if (dbrows($result)) {
		$data = dbarray($result);
		$str .= "<a href='".BASEDIR."articles.php'><strong>".$locale['404']."</strong></a>";
		$str .= " &raquo; <a href='".BASEDIR."articles.php?cat_id=".$_GET['cat_id']."'>".$data['article_cat_name']."</a>";
		$isTrue = true;
	}
}
...
if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
	...
	if (dbrows($result)) {
		...
		if ($data['article_allow_comments']) { showcomments("A", DB_ARTICLES, "article_id", $_GET['article_id'], BASEDIR."articles.php?article_id=".$_GET['article_id']); }
		if ($data['article_allow_ratings']) { showratings("A", $_GET['article_id'], BASEDIR."articles.php?article_id=".$_GET['article_id']); }
	} else {
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td valign='top' width='50%' class='tbl article_idx_cat_name'><!--article_idx_cat_name--><a href='".BASEDIR."articles.php?cat_id=".$data['article_cat_id']."'>".$data['article_cat_name']."</a> <span class='small2'>(".$data['article_count'].")</span>";
		...
		if (checkgroup($cdata['article_cat_access'])) {
			...
			if ($rows != 0) {
				...
				while ($data = dbarray($result)) {
					...
					echo "<div class='".$class."'><strong><a href='".BASEDIR."articles.php?article_id=".$data['article_id']."'>".$data['article_subject']."</a></strong>".$new."<br />\n".preg_replace("/<!?--\s*pagebreak\s*-->/i", "", stripslashes($data['article_snippet']))."</div>";
				echo ($i != $numrows ? "<hr />\n" : "\n"); $i++;
				}
