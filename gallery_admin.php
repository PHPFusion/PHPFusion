...
function gallery_photo_listing() {
    ...
    if (!empty($photoRows)) {
        ...
        echo "<strong>".$locale['album_0003']."</strong> ".parse_text($album_data['album_description'], FALSE, TRUE, FALSE);
...
function gallery_album_listing() {
    ...
    if (!empty($albumRows)) {
        ...
        ".(multilang_table("PG") ? "WHERE ".in_group('album_language', LANGUAGE)." AND " : "WHERE ").groupaccess('album.album_access')."
