...
add_to_title($locale['global_200'].$locale['blog_400']);

opentable($locale['blog_400']);
if (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) {
	$res = 0;
	$result = dbquery("SELECT blog_cat_name FROM ".DB_BLOG_CATS." ".(multilang_table("BL") ? "WHERE blog_cat_language='".LANGUAGE."' AND" : "WHERE")." blog_cat_id='".$_GET['cat_id']."'");
	if (dbrows($result) || $_GET['cat_id'] == 0) {
		...
		if ($rows) {
			...
			if ($_GET['cat_id'] != 0) {
				echo "<tr>\n<td width='150' class='tbl1' style='vertical-align:top'><!--blog_cat_image--><img class='blog-category' src='".get_image("bc_".$data['blog_cat_name'])."' alt='".$data['blog_cat_name']."' /><br /><br />\n";
				echo "<strong>".$locale['blog_401']."</strong> ".$data['blog_cat_name']."<br />\n<strong>".$locale['blog_402']."</strong> $rows</td>\n";
				echo "<td class='tbl1' style='vertical-align:top'>\n";
			} else {
				echo "<tr>\n<td width='150' class='tbl1' style='vertical-align:top'>".$locale['blog_403']."<br />\n";
				echo "<strong>".$locale['blog_401']."</strong> $rows</td>\n<td class='tbl1' style='vertical-align:top'><!--blog_cat_blog-->\n";
			}
			...
			echo "</td>\n</tr>\n<tr>\n<td colspan='2' class='tbl1' style='text-align:center'>".THEME_BULLET." <a href='".BASEDIR."blog_cats.php'>".$locale['blog_406']."</a>";
			echo "</td>\n</tr>\n</table><!--sub_blog_cat-->\n";
		}
	}
	...
	if (dbrows($result)) {
		...
		while ($data = dbarray($result)) {
			...
			if ($rows) {
				...
				while ($data2 = dbarray($result2)) {
					echo THEME_BULLET." <a href='".BASEDIR."blog.php?readmore=".$data2['blog_id']."'>".$data2['blog_subject']."</a><br />\n";
				}
				if ($rows > 10) { echo "<div style='text-align:right'>".THEME_BULLET." <a href='".BASEDIR."blog_cats.php?cat_id=".$data['blog_cat_id']."'>".$locale['blog_405']."</a></div>\n"; }
			} else {
				echo THEME_BULLET." ".$locale['blog_404']."\n";
			}
			echo "</td>\n</tr>\n";
		}
		$res = 1;
	...
	if (dbrows($result)) {
		...
		echo "<tr>\n<td width='150' class='tbl1' style='vertical-align:top'>".$locale['blog_403']."<br />\n";
		echo "<strong>".$locale['blog_402']."</strong> $nrows</td>\n<td class='tbl1' style='vertical-align:top'>\n";
		while ($data = dbarray($result)) {
			echo THEME_BULLET." <a href='".BASEDIR."blog.php?readmore=".$data['blog_id']."'>".$data['blog_subject']."</a><br />\n";
		}
		$res = 1;
		if ($nrows > 10) { echo "<div style='text-align:right'>".THEME_BULLET." <a href='".BASEDIR."blog_cats.php?cat_id=0'>".$locale['blog_405']."</a></div>\n"; }
		echo "</td>\n</tr>\n";
	}
	if ($res == 1) {
		echo "</table><!--sub_blog_cat_idx-->\n";
	} else {
		echo "<div style='text-align:center'><br />\n".$locale['blog_407']."<br /><br />\n</div>\n";
	}
}
closetable();
