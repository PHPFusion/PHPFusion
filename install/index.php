...
if ($settings) {
    ...
    if (INSTALLATION_STEP != 8 and dbresult(dbquery('SELECT exists(SELECT * FROM '.DB_PREFIX.'users)'),
                                            0) and intval($userdata['user_level']) !== USER_LEVEL_SUPER_ADMIN
    ) {
...
switch (INSTALLATION_STEP) {
    ...
    default:
        ...
        if (isset($db_prefix) && $db_prefix) {
            ...
            $content .= form_select('localeset', '', LANGUAGE, array(
                'options' => array_combine($locale_files, $locale_files),
