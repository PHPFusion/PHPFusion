...
if (isset($_POST['save_group'])) {
	...
	if ($check_count > 0) {
		...
		while ($data = dbarray($result)) {
			$user_groups = preg_replace(array("(^\.{$_GET['group_id']}$)", "(\.{$_GET['group_id']}\.)",
											  "(\.{$_GET['group_id']}$)"), array("", ".", ""), $data['user_groups']);
			$result2 = dbquery("UPDATE ".DB_USERS." SET user_groups='$user_groups' WHERE user_id='".$data['user_id']."'");
		}
	...
	while ($data = dbarray($result)) {
		...
		$user_groups = preg_replace(array("(^\.{$_GET['group_id']}$)", "(\.{$_GET['group_id']}\.)",
										  "(\.{$_GET['group_id']}$)"), array("", ".", ""), $user_groups);
		$result2 = dbquery("UPDATE ".DB_USERS." SET user_groups='$user_groups' WHERE user_id='".$data['user_id']."'");
	}
...
if (dbrows($result)) {
	...
	echo form_select('', 'group_id', 'group_id', $sel_opts, '', array('placeholder' => $locale['choose'],
																	  'class' => 'pull-left'));
