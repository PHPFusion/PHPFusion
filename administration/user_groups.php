...
if (isset($_POST['save_group'])) {
    ...
    if ($check_count > 0) {
        ...
        while ($data = dbarray($result)) {
            $user_groups = preg_replace(array(
                                            "(^\.{$_GET['group_id']}$)",
                                            "(\.{$_GET['group_id']}\.)",
                                            "(\.{$_GET['group_id']}$)"
                                        ), array(
...
if (dbrows($result)) {
    ...
    echo form_select('group_id', '', '', array(
        'options' => $sel_opts,
...
if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
    ...
    if (!isset($_POST['search_users'])) {
        echo openform('searchform', 'post', FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id'], array(
            'max_tokens' => 1,
            'notice' => 0
        ));
        echo "<table cellpadding='0' cellspacing='0' class='table table-responsive center'>\n";
    ...
    if (isset($_POST['search_users']) && isset($_POST['search_criteria'])) {
        ...
        if (isset($result) && dbrows($result)) {
            echo openform('add_users_form', 'post', FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id'], array(
                'max_tokens' => 1,
                'notice' => 0
            ));
    ...
    echo openform('rem_users_form', 'post', FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id'], array(
        'max_tokens' => 1,
        'notice' => 0
    ));
    echo "<table cellpadding='0' cellspacing='1' class='table table-responsive tbl-border center'>\n";
    ...
    if ($rows > 20) {
        echo "<div align='center' style='margin-top:5px;'>\n".makePageNav($_GET['rowstart'], 20, $rows, 3,
                                                                          FUSION_SELF.$aidlink."&amp;group_id=".$_GET['group_id']."&amp;")."\n</div>\n";
    }
    closetable();
