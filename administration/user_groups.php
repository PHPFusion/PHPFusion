...
if (isset($_GET['status']) && !isset($message)) {
    if ($_GET['status'] == "su") {
        $message = $locale['400'];
    } else if ($_GET['status'] == "sn") {
        $message = $locale['401'];
    } else if ($_GET['status'] == "remsel") {
        $message = $locale['402'];
    } else if ($_GET['status'] == "remall") {
        $message = $locale['403'];
    } else if ($_GET['status'] == "addsel") {
        $message = $locale['404'];
    } else if ($_GET['status'] == "deln") {
        $message = $locale['405']."<br />\n<span class='small'>".$locale['406']."</span>";
    } else if ($_GET['status'] == "dely") {
        $message = $locale['407'];
    }
    if ($message) {
...
if (isset($_POST['save_group'])) {
    ...
    if ($group_name) {
        if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
            ...
            if (db_lastid() == 101) {
                $result2 = dbquery("UPDATE ".DB_USER_GROUPS." SET group_id='104' WHERE group_id='101'");
            }
...
} else if (isset($_POST['add_sel']) && isnum($_GET['group_id'])) {
    $user_ids = "";
    $check_count = 0;
...
} else if (isset($_POST['remove_sel']) && isnum($_GET['group_id'])) {
    $user_ids = "";
    $check_count = 0;
    ...
    if ($check_count > 0) {
        ...
        while ($data = dbarray($result)) {
            $user_groups = preg_replace(["(^\.{$_GET['group_id']}$)", "(\.{$_GET['group_id']}\.)", "(\.{$_GET['group_id']}$)"], ["", ".", ""], $data['user_groups']);
            $result2 = dbquery("UPDATE ".DB_USERS." SET user_groups='$user_groups' WHERE user_id='".$data['user_id']."'");
        }
...
} else if (isset($_POST['remove_all']) && isnum($_GET['group_id'])) {
    ...
    while ($data = dbarray($result)) {
        ...
        $user_groups = preg_replace(["(^\.{$_GET['group_id']}$)", "(\.{$_GET['group_id']}\.)", "(\.{$_GET['group_id']}$)"], ["", ".", ""], $user_groups);
        $result2 = dbquery("UPDATE ".DB_USERS." SET user_groups='$user_groups' WHERE user_id='".$data['user_id']."'");
    }
    redirect(FUSION_SELF.$aidlink."&status=remall");
} else if (isset($_POST['delete']) && isnum($_GET['group_id'])) {
    ...
    if (dbrows($result)) {
        ...
        while ($data = dbarray($result)) {
            if (isset($_GET['group_id']) && isnum($_GET['group_id']))
                $sel = ($_GET['group_id'] == $data['group_id'] ? " selected='selected'" : "");
    ...
    if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
        ...
        if (isset($_POST['search_users']) && isset($_POST['search_criteria'])) {
            ...
            foreach ($search_items as $item) {
                ...
                } else if ($_POST['search_type'] == "user_name" && preg_match("/^[-0-9A-Z_@\s]+$/i", $item)) {
            ...
            } else if ($_POST['search_type'] == "user_name" && $search_names) {
