...
\PHPFusion\BreadCrumbs::getInstance()->addBreadCrumb(['link' => ADMIN.'administrators.php'.fusion_get_aidlink(), 'title' => $locale['AD']]);
...
if (isset($_GET['edit']) && isnum($_GET['edit']) && $_GET['edit'] != 1) {
    ...
    if (dbrows($result)) {
        ...
        $counter = 0;
        $page = 1;
        $admin_page = [$locale['441'], $locale['442'], $locale['443'], $locale['449'], $locale['444']];
    ...
    if (!isset($_POST['search_users']) || !isset($_POST['search_criteria'])) {
        ...
        } else if ($_POST['search_type'] == "user_name" && preg_match("/^[-0-9A-Z_@\s]+$/i", $_POST['search_criteria'])) {
