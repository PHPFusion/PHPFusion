	...
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		...
		while ($data = dbarray($result)) {
			...
			if (isset($_POST['reset_login']) && $_POST['reset_login'] == 1) {
				...
				$message = str_replace(array("[USER_NAME]", "[NEW_PASS]", "[NEW_ADMIN_PASS]", "[ADMIN]",
											 "[RESET_MESSAGE]"), array($data['user_name'], $newLoginPass, $newAdminPass,
																	   $userdata['user_name'],
																	   $reset_message), $locale['409']);
				$loginPassIsReset = ($loginPass->isValidNewPassword() === 0 ? TRUE : FALSE);
			} else {
				$message = str_replace(array("[USER_NAME]", "[NEW_ADMIN_PASS]", "[ADMIN]",
											 "[RESET_MESSAGE]"), array($data['user_name'], $newAdminPass,
																	   $userdata['user_name'],
...
echo openform('admin_reset', 'admin_reset', 'POST', FUSION_SELF.$aidlink);
...
echo "<td class='tbl1'>".form_select('', 'reset_admin', 'reset_admin', $reset_opts, '', array('placeholder' => $locale['choose'],
																							  'allowclear' => 1));
echo "</td>\n";
echo "</tr>\n<tr>\n";
