...
class AdminPasswordResetAdministration {
    ...
    public function __construct() {
        $this->locale = fusion_get_locale("", LOCALE.LOCALESET."admin/admin_reset.php");
        ...
        if (post('reset_id')) {
            ...
            addNotice('success', $this->locale['apw_429']);
        ...
        \PHPFusion\BreadCrumbs::getInstance()->addBreadCrumb(['link' => ADMIN.'admin_reset.php'.fusion_get_aidlink(), 'title' => $this->locale['apw_title']]);
        $this->set_adminsdb();
    }
        ...
        if (post('reset_admins')) {
            ...
            if (\Defender::safe()) {
                ...
                while ($data = dbarray($result)) {
                    ...
                    if ($loginPassIsReset && $adminPassIsReset && sendemail($data['user_name'], $data['user_email'], $userdata['user_name'], $userdata['user_email'], $this->locale['apw_407'].fusion_get_settings('sitename'), $message)) {
                ...
                if (!empty($reset_success)) {
                    foreach ($reset_success as $key => $info) {
                        ...
                        $text .= "<tr>\n";
                        $text .= "<td class='col-xs-2'><strong>".($key == 0 ? $this->locale['apw_424'] : "")."</strong></td>\n";
                ...
                if (!empty($reset_failed)) {
                    foreach ($reset_failed as $key => $info) {
                        ...
                        $text .= "<tr>\n";
                        $text .= "<td class='col-xs-2'><strong>".($key == 0 ? $this->locale['apw_425'] : "")."</strong></td>\n";
                        $text .= "<td class='col-xs-2'>".$info['user_name']." (".$info['user_email'].")</td>\n";
                        $text .= "</tr>\n";
                    }
                }
                ...
                $text .= "</table>\n</div>";
                $preview_html = openmodal('apw_preview', $this->locale['apw_410']);
    ...
    public function admin_reset_form() {
        ...
        echo form_select('reset_admin', $this->locale['apw_400'], '', [
            'required'    => TRUE,
        ...
        echo form_textarea('reset_message', $this->locale['apw_404'], '', ['inline' => TRUE, 'required' => TRUE, 'autosize' => TRUE]);
        echo form_checkbox("reset_login", $this->locale['apw_405'], '', ["inline" => TRUE]);
        echo form_button('reset_admins', $this->locale['apw_406'], $this->locale['apw_406'], ['class' => 'btn-primary']);
        echo closeform();
        closeside();
    }
    ...
    private function admin_title() {
        return ["all" => $this->locale['apw_401'], "sa" => $this->locale['apw_402'], "a" => $this->locale['apw_403']];
    }
    ...
    private function admin_reset_listing() {
        ...
        if (!empty($all_admin_reset)) {
            ...
            echo "<thead><tr>\n";
            echo "<th>&nbsp;</th>\n";
            echo "<th><small><strong>".$this->locale['apw_417']."</strong></small></th>\n";
            echo "<th><small><strong>".$this->locale['apw_418']."</strong></small></th>\n";
            echo "<th><small><strong>".$this->locale['apw_419']."</strong></small></th>\n";
            echo "<th><small><strong>".$this->locale['apw_420']."</strong></small></th>\n";
            echo "<th><small><strong>".$this->locale['apw_421']."</strong></small></th>\n";
            echo "<th><small><strong>".$this->locale['apw_427']."</strong></small></th>\n";
            echo "</tr></thead>\n";
            echo "<tbody>\n";
            ...
            foreach ($all_admin_reset as $info) {
                ...
                echo "<td>".$sucess." ".$this->locale['apw_422']." ".($sucess + $failed)."</td>\n";
                echo "<td>".($info['reset_reason'] ? $info['reset_reason'] : $this->locale['apw_423'])."</td>\n";
                echo "<td><a id='confirm' class='btn btn-danger btn-sm' href='".FUSION_SELF.fusion_get_aidlink()."&amp;section=adminreset_list&amp;action=delete&amp;reset_id=".$info['reset_id']."' onclick=\"return confirm('".$this->locale['apw_428']."');\"><i class='fa fa-trash'></i> ".$this->locale['delete']."</a></td>\n";
                echo "</tr>\n";
            }
            ...
            echo "<div class='display-inline-block pull-left m-r-20'>".form_checkbox('check_all', $this->locale['apw_430'], '', ['class' => 'm-b-0', 'reverse_label' => TRUE])."</div>";
            echo "<div class='display-inline-block'><a class='btn btn-danger btn-sm' onclick=\"run_admin('delete');\"><i class='fa fa-trash-o'></i> ".$this->locale['delete']."</a></div>";
            echo "</div>\n";
            echo closeform();
            add_to_jquery("
            ...
            echo "<div class='well text-center'>".$this->locale['apw_426']."</div>\n";
        }
        echo '</div>';
    }
