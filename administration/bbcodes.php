...
$section = get( 'section' );
$section = ( $section && in_array( $section, $allowed_section ) ? $section : 'bbcode_form' );
if ( $section == 'bbcode_list' ) {
    add_breadcrumb( [ 'link' => ADMIN.'bbcodes.php'.fusion_get_aidlink(), 'title' => $locale['BBCA_400'] ] );
}
...
echo opentab( $tab_title, $section, 'bbcode_list', TRUE, 'nav-tabs m-b-15' );
switch ( $section ) {
    case "bbcode_form":
        bbcode_form();
        break;
...
function bbcode_form() {
    ...
    if ( $action == 'mup' && $bbcode_id && $order ) {
        ...
        if ( substr( $enable, 0, 1 ) != '!' ) {
            ...
            dbquery( "INSERT INTO ".DB_BBCODES." (bbcode_name, bbcode_order) VALUES ('".$enable."', '".$order."')" );
        } else {
