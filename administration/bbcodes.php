...
function bbcode_list() {
    ...
    if ( check_get( 'action' ) && ( check_get( 'bbcode_id' ) && get( 'bbcode_id', FILTER_SANITIZE_NUMBER_INT ) ) ) {
    ...
    if ( dbrows( $result ) ) {
        ...
        echo "<table id='" . $fusion_table_id . "' class='table table-striped table-bordered'><thead><tr>";
        echo "<th></th>";
        ...
        while ( $data = dbarray( $result ) ) {
            ...
            if ($numrows != 1 ) {
                ...
                if ($i == 1) {
                    $up_link = clean_request( "section=bbcode_list&action=mdown&bbcode_id=" . $data['bbcode_id']."&order=" . $down, ['section', 'order', 'action', 'bbcode_id'], FALSE );
                    $up_down = " <a href='" . $up_link . "'>" . get_icon( 'fa fa-angle-down', 'fa-lg', $locale['BBCA_408'] ) . "</a>\n";
                } else {
                    $up_link = clean_request( "section=bbcode_list&action=mup&bbcode_id=" . $data['bbcode_id']."&order=" . $up, ['section', 'order', 'action', 'bbcode_id'], FALSE );
                    $up_down = " <a href='" . $up_link . "'>" . get_icon( 'fa fa-angle-up', 'fa-lg', $locale['BBCA_409'] ) . "</a>\n";
                    if ($i < $numrows) {
                        $up_link = clean_request( "section=bbcode_list&action=mdown&bbcode_id=" . $data['bbcode_id']."&order=" . $down, ['section', 'order', 'action', 'bbcode_id'], FALSE );
                        $up_down .= " <a href='" . $up_link . "'>" . get_icon( 'fa fa-angle-down', 'fa-lg', $locale['BBCA_408'] ) . "</a>\n";
                    }
            ...
            $del_link = clean_request( "section=bbcode_list&disable=".$data['bbcode_id'], ['section', 'disable', 'action', 'bbcode_id'], FALSE );

            echo "<tr data-id='" . $data['bbcode_id'] . "'>\n";
            echo "<td class='text-center'><a class='btn btn-default' href='" . $del_link . "'>" . $locale['BBCA_410'] . "</a></td>\n";
    ...
    if ( count( $available_bbcodes ) != $enabled ) {
        ...
        foreach ( $available_bbcodes as $available_bbcode ) {
            if ( !in_array( $available_bbcode, $enabled_bbcodes ) ) {
                ...
                $enable_link = clean_request( "section=bbcode_list&enable=" . $available_bbcode, ['section', 'enable'], FALSE );
                ...
                echo "<td class='text-center'><a href='" . $enable_link . "'>" . $locale['BBCA_414'] . "</a></td>\n";
                echo "</tr>\n";
                unset ( $__BBCODE__ );
            }
        }
