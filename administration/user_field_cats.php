...
if (!checkrights("UFC") || !defined("iAUTH") || !isset($_GET['aid']) || $_GET['aid'] != iAUTH) {
	redirect("../index.php");
}
...
if (isset($_GET['action']) && $_GET['action'] == "refresh") {
	...
	if (isset($_POST['savecat'])) {
		...
		$cat_class = form_sanitizer($_POST['cat_class'], '');
		$cat_db = form_sanitizer($_POST['cat_db'], '');
		$cat_index = form_sanitizer($_POST['cat_index'], '');
		...
		if (!defined('FUSION_NULL')) {
			if ((!$cat_db && !$cat_index) || ($cat_db && $cat_index)) {
				...
				$defender->stop();
				$defender->addNotice($locale['450b']);
			}
		}
	}
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	echo form_text($locale['420'], 'cat_name', 'cat_name', $cat_name, array('required' => 1, 'error_text' => $locale['450a'], 'inline'=>1));
	echo form_text($locale['423'], 'cat_db', 'cat_db', $cat_db, array('inline'=>1, 'placeholder'=>$locale['429a']));
	echo form_text($locale['428'], 'cat_index', 'cat_index', $cat_index, array('inline'=>1, 'placeholder'=>$locale['429b']));
	echo form_text($locale['425'], 'cat_class', 'cat_class', $cat_class, array('inline'=>1));
	echo form_text($locale['421'], 'cat_order', 'cat_order', $cat_order, array('number' => 1, 'width' => '100px', 'inline'=>1));
	echo form_toggle($locale['426'], 'cat_page', 'cat_page', array($locale['no'], $locale['yes']), $cat_page, array('inline'=>1));
	echo "<hr/>\n";
	echo "<div class='row'>\n<div class='col-sm-offset-3 col-md-offset-3 col-lg-offset-3 col-sm-9 col-md-9 col-lg-9 p-l-20'>\n";
	echo form_button($locale['422'], 'savecat', 'savecat', $locale['422'], array('class' => 'btn-primary'));
	echo "</div>\n</div>\n";
	echo closeform();
	closetable();
