	...
	if (!isset($_GET['ealbum_id'])) {
		...
		if (isset($_GET['status'])) {
			if ($_GET['status'] == "savepn") {
				...
				if ($_GET['error'] == 1) {
					$message .= $ESHPHOTOSL['422'];
				} elseif ($_GET['error'] == 2) {
					$message .= sprintf($ESHPHOTOSL['423'], parsebytesize($imagebytes));
				} elseif ($_GET['error'] == 3) {
					$message .= $ESHPHOTOSL['424'];
				} elseif ($_GET['error'] == 4) {
					$message .= sprintf($ESHPHOTOSL['425'], $imagewidth, $imageheight);
				}
			}
...
<input type='submit' name='save_photo' value='".$ESHPHOTOSL['447']."' class='button'>\n";
		...
		if ($rows) {
			$result = dbquery("SELECT tp.*, tu.user_id,user_name FROM ".DB_ESHOP_PHOTOS." tp
