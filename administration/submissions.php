...
if (!isset($_GET['action']) || $_GET['action'] == "1") {
    if (isset($_GET['delete']) && isnum($_GET['delete'])) {
        ...
        echo "<table class='table table-responsive tbl-border center' width='100%;'>\n<tbody>\n<tr>\n";
        ...
        echo "</tr>\n".$news."<tr>\n";
        echo "<td colspan='2' class='tbl2'><a id='blog_submissions' name='blog_submissions'></a>\n".$locale['412b']."</td>\n";
        echo "</tr>\n".$blog."<tr>\n";
    ...
    if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
        ...
        if (dbrows($result)) {
            ...
            if (dbrows($result2)) {
                while ($data2 = dbarray($result2)) {
                    if (isset($news_cat))
                        $sel = ($news_cat == $data2['news_cat_id'] ? " selected='selected'" : "");
...
if ((isset($_GET['action']) && $_GET['action'] == "2") && (isset($_GET['t']) && $_GET['t'] == "b")) {
    if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
        ...
        if (dbrows($result)) {
            ...
            $result = dbquery("INSERT INTO ".DB_PREFIX."blog (blog_subject, blog_cat, blog_blog, blog_extended, blog_breaks, blog_name, blog_datestamp, blog_start, blog_end, blog_visibility, blog_reads, blog_allow_comments, blog_allow_ratings, blog_language) VALUES ('$blog_subject', '$blog_cat', '$blog_snippet', '$blog_body', '$blog_breaks', '".$data['user_id']."', '".time()."', '0', '0', '0', '0', '1', '1' ,'".LANGUAGE."')");
            $result = dbquery("DELETE FROM ".DB_SUBMISSIONS." WHERE submit_id='".$_GET['submit_id']."'");
        ...
        if (dbrows($result)) {
            ...
            $blog_cat_opts = "";
            $sel = "";
            $result2 = dbquery("SELECT blog_cat_id, blog_cat_name FROM ".DB_PREFIX."blog_cats ORDER BY blog_cat_name");
            if (dbrows($result2)) {
                while ($data2 = dbarray($result2)) {
                    if (isset($blog_cat))
                        $sel = ($blog_cat == $data2['blog_cat_id'] ? " selected='selected'" : "");
    ...
    if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
        ...
        if (dbrows($result)) {
            ...
            if (!empty($submit_criteria['download_image']) && !empty($submit_criteria['download_image_thumb'])) {
                ...
                echo "<img src='".DOWNLOADS."submissions/images/".$submit_criteria['download_image_thumb']."' alt='img' /></a><br />\n";
    ...
    if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
        ...
        if (dbrows($result)) {
            ...
            if (dbrows($result2)) {
                while ($data2 = dbarray($result2)) {
                    if (isset($album_id))
                        $sel = ($album_id == $data2['album_id'] ? " selected='selected'" : "");
