...
// Manual updating of the system
$batcher = Batch::getInstance();

$to_upgrade = $batcher->checkUpgrades();
...
if (!empty($to_upgrade)) {
    ...
    if (check_get('update')) {
                ...
                if (!empty($file_upgrades)) {
                    ...
                    foreach ($file_upgrades as $callback_method => $upgrades) {
                        if (!empty($upgrades)) {
                            ...
                            if (method_exists($batcher, $method)) {
                                Infusions::getInstance()->$method($file_upgrades);
                            }
                        }
                    }
                }
        ...
        redirect(clean_request('', ['update'], FALSE));
    }

?>
    <div class="alert alert-default display-flex align-items-center justify-content-between">New local upgrades available. Do you wish to update your system now?<a href="<?php echo clean_request('update=1', [], false) ?>" class="btn btn-primary">Upgrade Now</a></div>
<?php
}
