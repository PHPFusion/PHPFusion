...
echo opentab( $tabs, $sections, 'blacklist', TRUE, 'nav-tabs', 'section', ['rowstart', 'action', 'blacklist_id'] );
switch ( $sections ) {
    case 'blacklist_form':
        blacklist_form();
        break;
...
function blacklist_listing() {
    ...
    echo "<a class='btn btn-success btn-sm m-r-10' href=" . clean_request( 'section=blacklist_form', ['section', 'rowstart'], FALSE ) . ">" . get_icon( 'fa fa-plus', 'fa-fw m-r-10' ) . $locale['BLS_022'] . "</a>";
    if ( !empty( $total_rows ) ) {
    ...
    if ( $rows > 0 ) {
        ...
        while ( $data = dbarray( $result ) ) {
            $edit_link = clean_request( "section=blacklist_form&action=edit&blacklist_id=" . $data['blacklist_id'], ['section', 'ref', 'action', 'blacklist_id'], FALSE );
            $delete_link = clean_request( "section=blacklist&action=delete&blacklist_id=" . $data['blacklist_id'], ['section', 'ref', 'action', 'blacklist_id'], FALSE );
            ...
            echo "<td>
                <a class='btn btn-default btn-sm' href='" . $edit_link . "'>" . get_icon( 'fa fa-edit', 'fa-fw m-r-10' ) . $locale['edit'] . "</a>
                <a class='btn btn-danger btn-sm' href='" . $delete_link . "' onclick=\"return confirm('" . $locale['BLS_014'] . "');\">" . get_icon( 'fa fa-trash', 'fa-fw m-r-10' ) . $locale['delete'] . "</a>
                </td>\n";
            echo "</tr>\n";
