...
\PHPFusion\BreadCrumbs::getInstance()->addBreadCrumb(['link' => ADMIN.'articles.php'.fusion_get_aidlink(), 'title' => $locale['A']]);
...
if (!empty($result)) {
    if (isset($_POST['save'])) {
        ...
        if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_POST['article_id']) && isnum($_POST['article_id'])) || (isset($_GET['article_id']) && isnum($_GET['article_id']))) {
            ...
            if (isset($_POST['article_id']) && isnum($_POST['article_id'])) {
                $id = $_POST['article_id'];
            } else if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
                $id = $_GET['article_id'];
            }
        ...
        while ($data = dbarray($result)) {
            if (isset($article_cat))
                $sel = ($article_cat == $data['article_cat_id'] ? " selected='selected'" : "");
