...
if (isset($_POST['save'])) {
    ...
    if (isset($_FILES['blog_image']) && is_uploaded_file($_FILES['blog_image']['tmp_name'])) {
        ...
        if (!preg_match("/^[-0-9A-Z_\.\[\]]+$/i", $image_name)) {
            $error = 1;
        } else if ($image['size'] > $settings['blog_photo_max_b']) {
            $error = 2;
        } else if (!$filetype) {
            $error = 3;
        } else {
            ...
            if (function_exists("chmod")) {
                chmod(IMAGES_B.$image_full, 0644);
            }
    ...
    if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_POST['blog_id']) && isnum($_POST['blog_id'])) || (isset($_GET['blog_id']) && isnum($_GET['blog_id']))) {
        ...
        if (dbrows($result)) {
            ...
            if ($data['blog_start'] > 0)
                $blog_start = getdate($data['blog_start']);
            if ($data['blog_end'] > 0)
                $blog_end = getdate($data['blog_end']);
    ...
    if (dbrows($result)) {
        while ($data = dbarray($result)) {
            if (isset($blog_cat))
                $sel = ($blog_cat == $data['blog_cat_id'] ? " selected='selected'" : "");
    ...
    for ($i = 1; $i <= 31; $i++)
        echo "<option".(isset($blog_start['mday']) && $blog_start['mday'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> <select name='blog_start[mon]' class='textbox'>\n<option>--</option>\n";
    for ($i = 1; $i <= 12; $i++)
        echo "<option".(isset($blog_start['mon']) && $blog_start['mon'] == $i ? " selected='selected'" : "").">$i</option>\n";
    ...
    for ($i = 0; $i <= 24; $i++)
        echo "<option".(isset($blog_start['hours']) && $blog_start['hours'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> : <select name='blog_start[minutes]' class='textbox'>\n";
    for ($i = 0; $i <= 60; $i++)
        echo "<option".(isset($blog_start['minutes']) && $blog_start['minutes'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> : 00 ".$locale['429']."</td>\n";
    echo "</tr>\n<tr>\n";
    echo "<td class='tbl'>".$locale['428']."</td>\n";
    echo "<td class='tbl'><select name='blog_end[mday]' class='textbox'>\n<option>--</option>\n";
    for ($i = 1; $i <= 31; $i++)
        echo "<option".(isset($blog_end['mday']) && $blog_end['mday'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> <select name='blog_end[mon]' class='textbox'>\n<option>--</option>\n";
    for ($i = 1; $i <= 12; $i++)
        echo "<option".(isset($blog_end['mon']) && $blog_end['mon'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> <select name='blog_end[year]' class='textbox'>\n<option>----</option>\n";
    for ($i = (isset($blog_end['year']) && $blog_end['year'] != "----" ? $blog_end['year'] : date('Y')); $i <= date("Y", strtotime('+10 years')); $i++)
        echo "<option".(isset($blog_end['year']) && $blog_end['year'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> / <select name='blog_end[hours]' class='textbox'>\n";
    for ($i = 0; $i <= 24; $i++)
        echo "<option".(isset($blog_end['hours']) && $blog_end['hours'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> : <select name='blog_end[minutes]' class='textbox'>\n";
    for ($i = 0; $i <= 60; $i++)
        echo "<option".(isset($blog_end['minutes']) && $blog_end['minutes'] == $i ? " selected='selected'" : "").">$i</option>\n";
    echo "</select> : 00 ".$locale['429']."</td>\n";
    echo "</tr>\n<tr>\n";
