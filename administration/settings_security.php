...
if (isset($_POST['savesettings'])) {
	...
	$StoreArray = array("captcha" => form_sanitizer($_POST['captcha'], "", "captcha"),
	...
	if ($StoreArray['captcha'] == "grecaptcha") {
		...
		$StoreArray += array("recaptcha_public" => form_sanitizer($_POST['recaptcha_public'], "", "recaptcha_public"),
...
if (!$recaptcha_upgrade) {
	$newSettings = array("settings_name" => "recaptcha_type",
		"settings_value" => "text",);
...
openside('');
echo form_select('captcha', $locale['693'], fusion_get_settings('captcha'), array('options' => $available_captchas,
	'inline' => TRUE));
...
echo form_text('recaptcha_public', $locale['grecaptcha_0100'], fusion_get_settings('recaptcha_public'), array("inline" => TRUE,
	"placeholder" => $locale['grecaptcha_placeholder_1'],
	"required" => TRUE)); // site key
echo form_text('recaptcha_private', $locale['grecaptcha_0101'], fusion_get_settings('recaptcha_private'), array("inline" => TRUE,
	"placeholder" => $locale['grecaptcha_placeholder_2'],
	"required" => TRUE)); // secret key
echo form_select('recaptcha_theme', $locale['grecaptcha_0102'], fusion_get_settings('recaptcha_theme'), array("options" => array("light" => $locale['grecaptcha_0102a'],
	"dark" => $locale['grecaptcha_0102b']),
	"inline" => TRUE));
echo form_select('recaptcha_type', $locale['grecaptcha_0103'], fusion_get_settings('recaptcha_type'), array("options" => array("text" => $locale['grecaptcha_0103a'],
...
closeside();
openside('');
$level_array = array(USER_LEVEL_ADMIN => $locale['676'],
	USER_LEVEL_SUPER_ADMIN => $locale['677'],
	USER_LEVEL_MEMBER => $locale['678']);
echo form_select('maintenance_level', $locale['675'], fusion_get_settings('maintenance_level'), array('options' => $level_array,
	'inline' => 1,
	'width' => '100%'));
$opts = array('1' => $locale['502'], '0' => $locale['503']);
echo form_select('maintenance', $locale['657'], fusion_get_settings('maintenance'), array('options' => $opts,
	'inline' => 1,
	'width' => '100%'));
echo form_textarea('maintenance_message', $locale['658'], fusion_get_settings('maintenance_message'), array('autosize' => 1));
closeside();
openside('');
echo form_textarea('privacy_policy', $locale['820'], fusion_get_settings('privacy_policy'), array('autosize' => 1,
...
echo form_select('flood_autoban', $locale['680'], fusion_get_settings('flood_autoban'), array('options' => $flood_opts,
	'width' => '100%'));
closeside();
openside('');
...
echo form_textarea('bad_words', $locale['651'], fusion_get_settings('bad_words'), array('placeholder' => $locale['652'],
	'autosize' => 1));
closeside();
echo "</div>\n</div>\n";
