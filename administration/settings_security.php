...
closeside();
openside('');
$level_array = array(USER_LEVEL_ADMIN => $locale['676'], USER_LEVEL_SUPER_ADMIN => $locale['677'], '1' => $locale['678']);
...
echo form_textarea($locale['658'], 'maintenance_message', 'maintenance_message', $settings['maintenance_message'], array('autosize'=>1));
closeside();
openside('');
echo form_textarea($locale['820'], 'privacy_policy', 'privacy_policy', $settings['privacy_policy'], array('autosize'=>1, 'form_name'=>'settingsform', 'html'=>!$settings['tinymce_enabled'] ? 1 : 0));
closeside();
