...
openside('');
echo form_select('captcha', $locale['693'], $available_captchas, $settings['captcha'], array('inline'=>1));
...
echo form_select('recaptcha_theme', $locale['697'], $theme_opts, $settings['recaptcha_theme']);
echo "</div>\n";
closeside();
openside('');
$level_array = array(USER_LEVEL_ADMIN => $locale['676'], USER_LEVEL_SUPER_ADMIN => $locale['677'], USER_LEVEL_MEMBER => $locale['678']);
echo form_select('maintenance_level', $locale['675'], $level_array, $settings['maintenance_level'], array('inline'=>1, 'width'=>'100%'));
$opts = array('1' => $locale['502'], '0' => $locale['503']);
echo form_select('maintenance', $locale['657'],  $opts, $settings['maintenance'], array('inline'=>1, 'width'=>'100%'));
...
echo form_select('flood_autoban', $locale['680'], $flood_opts, $settings['flood_autoban'], array('width'=>'100%'));
closeside();
openside('');
$yes_no_array = array('1' => $locale['518'], '0' => $locale['519']);
echo form_select('bad_words_enabled', $locale['659'], $yes_no_array, $settings['bad_words_enabled']);
