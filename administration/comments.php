...
if (isset($_POST['save_comment']) && (isset($_GET['comment_id']) && isnum($_GET['comment_id']))) {
    ...
    dbquery("UPDATE ".DB_COMMENTS." SET comment_message='".$comment_message."' WHERE comment_id='".$_GET['comment_id']."'");
    addNotice('success', $locale['410']);
    redirect(clean_request('', ['comment_id'], FALSE));
}

if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['comment_id']) && isnum($_GET['comment_id']))) {
    dbquery("DELETE FROM ".DB_COMMENTS." WHERE comment_id='".$_GET['comment_id']."'");
    addNotice('success', $locale['411']);
    redirect(clean_request('', ['action', 'comment_id'], FALSE));
}
openside(\PHPFusion\Admins::getInstance()->getCommentType($_GET['ctype'])." ".$locale['401']);
...
if (dbrows($result)) {
    while ($data = dbarray($result)) {
        ...
        echo "<a class='btn btn-xs btn-default' href='".FUSION_SELF.fusion_get_aidlink()."&amp;ctype=".$_GET['ctype']."&amp;comment_item_id=".$_GET['comment_item_id']."&amp;action=edit&amp;comment_id=".$data['comment_id']."'>".$locale['430']."</a>";
        echo "<a class='btn btn-xs btn-default' href='".FUSION_SELF.fusion_get_aidlink()."&amp;ctype=".$_GET['ctype']."&amp;comment_item_id=".$_GET['comment_item_id']."&amp;action=delete&amp;comment_id=".$data['comment_id']."' onclick=\"return confirm('".$locale['433']."');\">".$locale['431']."</a>";
        echo "</div>\n";
