...
if (isset($_POST['savesettings'])) {
    ...
    if (\defender::safe()) {
        foreach ($settings_misc as $settings_name => $settings_value) {
            ...
            dbquery_insert(DB_SETTINGS, $data, "update", array("primary_key" => "settings_name", 'keep_session' => TRUE));
        }
...
echo form_checkbox('rendertime_enabled', $locale['688'], $settings_misc['rendertime_enabled'], array(
    'options' => $opts,
    'inline'  => TRUE,
    'type'    => 'radio',
));
...
openside('');
echo form_checkbox('comments_enabled', $locale['671'], $settings_misc['comments_enabled'], array(
    'inline'  => TRUE,
    'options' => $choice_arr,
    'type'    => 'radio'
));
echo form_text('comments_per_page', $locale['913'], $settings_misc['comments_per_page'], array('inline' => TRUE, 'error_text' => $locale['error_value'], 'type' => 'number', 'inner_width' => '150px'));

$sort_opts = array('ASC' => $locale['685'], 'DESC' => $locale['686']);
echo form_checkbox('comments_sorting', $locale['684'], $settings_misc['comments_sorting'], array(
    'inline'  => TRUE,
    'options' => $sort_opts,
    'type'    => 'radio',
));
