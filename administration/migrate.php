        ...
        if (dbrows($result) > 0) {
            ...
            if (dbrows($result2) > 0) {
                if (isset($_POST['forum']) == '1') {
                    user_posts_migrate($user_primary_id, $user_temp_id, DB_THREAD_NOTIFY, 'notify_user', $locale['102']);
                    user_posts_migrate($user_primary_id, $user_temp_id, DB_THREADS, 'thread_author', $locale['103']);
                    user_posts_migrate($user_primary_id, $user_temp_id, DB_THREADS, 'thread_lastuser', $locale['104']);
