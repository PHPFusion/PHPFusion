			...
			if ($i == 0) {
				...
				if (dbrows($result) > 0) {
					...
					while ($data = dbarray($result)) {
						...
						$html .= checkrights($data['admin_rights']) ? "<li $secondary_active><a href='".ADMIN.$data['admin_link'].$aidlink."'> ".$this->get_admin_icons($data['admin_rights'])." ".$locale[$data['admin_rights']]." ".$data['admin_rights']."</a></li>\n" : '';
					}
					$html .= "</ul>\n";
				}
				$html .= "</div>\n";
