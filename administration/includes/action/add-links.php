...
function add_links($data) {
    ...
    if (($data["link_position"] && isnum($data["link_position"])) && !empty($data["link_name"])) {
        $data["link_status"] = 0;
        $data["link_id"] = 0;
        $data["link_type"] = "link";
        ...
        echo openform("_form", "post", array("form_id"=>"_form_".$link_id));
        ...
        echo form_text("_title", "Title Attribute", '', array("input_id" => "_title_".$link_id, "required"=>true));
