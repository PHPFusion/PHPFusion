    ...
    if (!isset($_GET['download_id']) || !isnum($_GET['download_id'])) {
        ...
        if (dbrows($cat_list_result)) {
            ...
            if (isset($_POST['cat_id']) && isnum($_POST['cat_id']) && $_POST['cat_id'] != "all") {
                ...
                $order_by_allowed = array("download_id", 'download_title', "download_user", "download_count", "download_datestamp");
            ...
            echo form_text($locale['460'], 'stext', 'search_downloads', '', array('placeholder' => $locale['461'], 'append_button' => 1));
            ...
            echo form_select($locale['462'], 'cat_id', 'cat_id', $catlist_opts, isset($_GET['cat_id']) ? $_GET['cat_id'] : '', array('class' => 'pull-left', 'inline' => 1));
            add_to_jquery("
            $('#cat_id').select2().bind('change', function() { this.form.submit(); });
            ");
            if (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) {
                $order_opts = array('download_id' => $locale['452'], 'download_title' => $locale['453'], 'download_user' => $locale['454'], 'download_count' => $locale['455'], 'download_datestamp' => $locale['456']);
                ...
                echo form_select('', 'sort', 'sort_downloads', $sort_opts, $sort, array('class' => 'pull-right', 'width' => '150px'));
                echo form_select('', 'orderby', 'orderby_downloads', $order_opts, $order_by, array('class' => 'pull-right m-r-10', 'width' => '100px'));
