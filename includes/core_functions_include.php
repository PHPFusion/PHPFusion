 ...
 * Ensures only safe characters, strips invalid entities,
 * and restricts to whitelisted protocols.
 *
...
function cleanurl($url) {
    ...
    // Encode back for HTML context
    return htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
}

/**
 * Sanitizes a URL for safe usage in HTML attributes.
 *
 * Prevents javascript: or other unsafe protocols,
 * strips invalid characters, and encodes for safe output.
 *
 * @param string $url               The raw URL.
 * @param array  $allowed_protocols List of allowed schemes.
 *
 * @return string Sanitized URL, or empty string if unsafe.
 */
function stripurl($url, $allowed_protocols = [ 'http', 'https', 'mailto' ]) {

    if ( is_null($url) || $url === '' ) {
        return '';
    }
    // Trim and remove whitespace characters
    $url = trim($url);
    ...
    // Encode for safe HTML attribute output
    return htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
}
...
function fusion_parse_user($user_name, $tooltip = '') {
    ...
    return preg_replace_callback("/\@([A-Za-z0-9\-_!\.]+)/",
        function ($user_name) use ($tooltip) {
...
function pageaccess($rights, $debug = FALSE) {
    ...
    $request_aid = isset($_GET['aid']) ? (string)$_GET['aid'] : '';
    if (iADMIN && defined('iAUTH') && checkrights($rights) && ($request_aid === '' || !hash_equals(iAUTH, $request_aid))) {
    ...
    if ( iADMIN && $request_aid !== '' && defined('iAUTH') ) {
        if ( ! hash_equals(iAUTH, $request_aid) ) {
...
function getusergroups() {
    ...
    foreach ( $groups_cache as $group ) {
        array_push($groups_array, [ $group['group_id'], $group['group_name'], $group['group_description'], $group['group_icon'], $group['group_image'] ]);
    }
...
/**
 * Shift a user's group membership by removing one group ID
 * and adding another.
 *
 * Accepts a dotted string format like ".1.29.30.15.16."
 * and returns the same format after modification.
 *
 * @param string $group_string Current membership string
 * @param string $add          Group ID to add
 * @param string $remove       Group ID to remove
 *
 * @return string               Updated membership string
 */
function fusion_shift_group(string $group_string, string $add, string $remove)
: string {

    // Convert the dot-delimited string to an array
    $arr = array_filter(explode('.', $group_string));
    ...
    // Add new group if not already added
    if ( ! in_array($add, $arr) ) {
        $arr[] = $add;
    }

    // Optional: keep numeric sorting consistent
    sort($arr, SORT_NUMERIC);

    // Restore dotted format
    return '.' . implode('.', $arr);
}
...
function makepagenav($rowstart, $count, $total, $range = 3, $link = "", $getname = "rowstart", $button = FALSE) {
    ...
    $tpl_button = "<a class='btn btn-default btn-block btn-md' data-value='%d' href='%s=%d'>%s</a>\n";
    ...
    $cur_page_field = form_text($getname . '_pg', '', $cur_page, [ 'inline' => TRUE, 'class' => 'm-0' ]);
...
/**
 * Hierarchy Page Breadcrumbs, generates breadcrumbs on all categories
 *
 * @param        $tree_index    dbquery_tree() or tree_index().
 * @param        $tree_full     dbquery_tree_full().
 * @param        $id_col        The name of the category id column.
 * @param        $title_col     The name of the category nmae column.
 * @param        $link          The name of the $_GET parameter.
 * @param string $getname       Additions get name
 */
function make_page_breadcrumbs($tree_index, $tree_full, $id_col, $title_col, $link, $getname = "rownav") {

    $_crumb_get = get($getname, FILTER_VALIDATE_INT) ? : 0;

    // Recursive fatal protection
    if ( ! function_exists('breadcrumb_page_arrays') ) {
        function breadcrumb_page_arrays($tree_index, $tree_full, $id_col, $title_col, $link, $getname, $id) {

            $crumb = [];
            if ( isset($tree_index[ get_parent($tree_index, $id) ]) ) {
                $_name = get_parent_array($tree_full, $id);
                $crumb = [
                    'link'  => isset($_name[ $id_col ]) ? $link . "{$getname}={$_name[$id_col]}" : "", //clean_request($getname . "=" . $_name[$id_col], ['aid']) : '',
                    'title' => isset($_name[ $title_col ]) ? QuantumFields::parseLabel($_name[ $title_col ]) : "",
                ];
                if ( get_parent($tree_index, $id) == 0 ) {
                    return $crumb;
                }
                $crumb_1 = breadcrumb_page_arrays($tree_index, $tree_full, $id_col, $title_col, $link, $getname, get_parent($tree_index, $id));
                if ( ! empty($crumb_1) ) {
    ...
    $crumb = breadcrumb_page_arrays($tree_index, $tree_full, $id_col, $title_col, $link, $getname, $_crumb_get);
...
/**
 * Generates an Admin ID link string for authentication.
 *
 * @param bool $append If true, uses '&' to append to existing query strings.
 *                     If false, uses '?' to start a new query string.
 *
 * @return string Returns the formatted aid string or an empty string if not in admin mode.
 */
function fusion_get_aidlink($append = FALSE) {

    $aidlink = '';

    // Check if user is in the admin panel and authenticated
    if ( defined('iADMIN') && iADMIN && defined('iAUTH') ) {
        // Choose prefix based on whether we are appending to existing parameters
        $separator = ( $append === TRUE ) ? '&' : '?';
        $aidlink = $separator . 'aid=' . iAUTH;
    }
...
/**
 * Return the current request URL with the active admin aid token.
 */
function fusion_get_current_aid_url() {

    if ( ! defined('iAUTH') ) {
        return FUSION_REQUEST;
    }

    $request_path = strtok(FUSION_REQUEST, '?');
    $query = $_GET;
    $query['aid'] = iAUTH;

    return $request_path . '?' . http_build_query($query, '', '&', PHP_QUERY_RFC3986);
}
...
function valid_language($lang, $file_check = FALSE) {
    ...
    if (
        preg_match("/^([a-z0-9_-]){2,50}$/i", $lang) &&
 ...
 * @return string
 */
function calculate_hex($hex, $adjustment) {
    ...
    // For lightening (negative percent)
    if ( $percent < 0 ) {
        $r = max(0, min(255, $r - ( $r * $percent / 100 ))); // subtracting negative = lighten
        $g = max(0, min(255, $g - ( $g * $percent / 100 )));
        $b = max(0, min(255, $b - ( $b * $percent / 100 )));
    }
...
function fusion_load_script($file_path, $file_type = 'script', $html = FALSE) {
    ...
    if ( $file_path && empty($paths[ $file_path ]) ) {
        ...
        if ( ! empty($_fileinfo['extension']) ) {
            ...
            if ( $file_type == "script" || $file_type == 'js' ) {
            ...
            } else if ($file_type == "module") {
                $html_tag = "<script src='$file_path' type='module'></script>";
                if ( $html === TRUE ) {
                    return $html_tag;
                }
                add_to_footer($html_tag);
        ...
        if ( $file_type == 'connection' ) {
            $html_tag = "<link rel='dns-prefetch' href='$file_path'>\n";
            $html_tag .= "<link rel='preconnect' href='$file_path' crossorigin>";
...
/**
 * You can call a simple function when a user hits a specific page (like success.php) or clicks a "Buy" button via AJAX:
 */
function mark_session_conversion() {

    dbquery("UPDATE " . DB_SESSIONS_RATE . " SET is_conversion = 1 WHERE session_id = :sid",
            [
                ':sid' => SESSION_ID
            ]);
}
...
/**
 * Helper function to register a task locally
 *
 * @param string   $taskKey   Unique task identifier
 * @param callable $handler   Function to execute
 * @param int      $priority  Higher = runs first
 * @param int|null $recurring Optional recurring interval in seconds
 */
function register_local_task(string $taskKey, callable $handler, int $priority = 0, ?int $recurring = NULL)
: void {

    // Register tasks easily using the local helper
    // registerLocalTask(
    //     'send_reminder',
    //     fn($payload) => ReminderService::send($payload),
    //     priority: 10,
    //     recurring: 86400 // every 24 hours
    // );
    TaskDispatcher::register($taskKey, $handler, $priority, $recurring);
}
 ...
 *
 * @param string          $taskKey     Task identifier registered in TaskDispatcher
 * @param string|DateTime $runAt       DateTime string (Y-m-d H:i:s) or relative string like '+1 day'
 * @param array           $payload     Optional dynamic payload
...
: bool {
    ...
    if ( ! ( $runAt instanceof DateTime ) ) {
        try {
            $runAt = new DateTime($runAt);
        } catch ( Exception $e ) {
            throw new RuntimeException("Invalid runAt value: {$runAt}");
        }
    }
