 ...
 *
 * @param       $format  $format shortdate, longdate, forumdate, newsdate or date pattern for the strftime
...
function showdate( $format, $val, $options = [] ) {
    ...
    if ( !empty( $val ) ) {
        if ( in_array( $format, [ 'shortdate', 'longdate', 'forumdate', 'newsdate' ] ) ) {
            ...
            $offset = (int)$val + (int)$offset;

            return strftime( $format, (int)$offset );
        } else {
            $offset = (int)$val + (int)$offset;
