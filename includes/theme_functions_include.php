...
function show_dropdown(array $item, array $menu_items)
{
	...
	<div class="dropdown">
		...
		echo whitespace($item['class']) ?> dropdown-toggle" type="button" data-bs-toggle="dropdown"
				aria-expanded="false">
			<?php
			if ($item["icon"]) :
		...
		echo whitespace($item['menu_class']) ?>">
			<?php
			...
			foreach ($menu_items as $c_items) : ?>
				...
				<?php
					...
					<li<?php
					echo whitespace($c_items['li_class'] ? 'class="' . $c_items['li_class'] . '"' : '') ?>>
						<a class="dropdown-item<?php
						echo whitespace($c_items['link_class'] ?? '') ?>" href="<?php
						echo $c_items['link'] ?>"><?php
							echo $c_items['title'] ?></a>
					</li>
				<?php
				endif; ?>
...
if (!function_exists('tab_active')
	...
	class FusionTabs
	{
		 ...
		 * @return string
		 */
		public function openTab($tabs, $link_active_arrkey, $id, $link = FALSE, $class = FALSE, $getname = 'section', array $cleanup_get = [])
		{
			...
			return fusion_get_template('tabs', $info);
		}
		...
		public function openTabBody($id, $link_active_arrkey = NULL, $key = 'section')
		{
			...
			return fusion_get_template('tabs', [
				'id'     => $id,
				'part'   => 'openbody',
		...
		{
			return fusion_get_template('tabs', ['part' => 'closebody']);
