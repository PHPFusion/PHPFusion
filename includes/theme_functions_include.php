    ...
    function display_avatar(array $userdata, $size, $class = '', $link = TRUE, $img_class = '', $custom_avatar = '') {
        ...
        if ($hasAvatar) {
            ...
            if (!empty($custom_avatar) && file_exists($custom_avatar)) {
                ...
                $img = '<div class="avatar '.$img_class.'" style="width:'.$size.';max-height:'.$size.';"><svg version="1.1" viewBox="0 0 20 20"><rect fill="#'.$color.'" stroke-width="0" y="0" x="0" height="100%" width="100%"/><text fill="#'.$font_color.'" x="50%" y="50%" text-anchor="middle" alignment-baseline="central" dy="-0.05em">'.$first_char.'</text></svg></div>';
            }
        }
...
function newsopts($info, $sep, $class = "") {
    ...
    if (!isset($_GET['readmore']) && $info['news_ext'] == "y")
        $res = "<a href='news.php?readmore=".$info['news_id']."'".$link_class.">".$locale['global_072']."</a> ".$sep." ";
...
function blogopts($info, $sep, $class = "") {
    ...
    if (!isset($_GET['readmore']) && $info['blog_ext'] == "y")
        $res = "<a href='blog.php?readmore=".$info['blog_id']."'".$link_class.">".$locale['global_072']."</a> ".$sep." ";
    if ($info['blog_allow_comments'] && $settings['comments_enabled'] == "1") {
        $res .= "<a href='blog.php?readmore=".$info['blog_id']."#comments'".$link_class.">".$info['blog_comments'].($info['blog_comments'] == 1 ? $locale['global_073b'] : $locale['global_073'])."</a> ".$sep." ";
    }
    if ($info['blog_ext'] == "y" || ($info['blog_allow_comments'] && $settings['comments_enabled'] == "1")) {
        $res .= $info['blog_reads'].$locale['global_074']."\n ".$sep;
    }
...
function timer($updated = FALSE) {
    ...
    $timer = [$year => "year", $month => "month", $day => "day", $hour => "hour", $minute => "minute", $second => "second"];
...
function opentab($tab_title, $link_active_arrkey, $id, $link = FALSE, $class = '') {
    global $aidlink;
    $link_mode = $link == 1 ? 1 : 0;
    $html = "<div class='nav-wrapper'>\n";
    $html .= "<ul class='nav ".($class ? $class : 'nav-tabs')."' $id >\n";
