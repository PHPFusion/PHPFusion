...
+--------------------------------------------------------*/
/*
 * @todo The class should be moved into the "classes" folder.
 */
namespace PHPFusion {
	/**
...
namespace {
	...
	function get_image($image, $alt = "", $style = "", $title = "", $atts = "") {
		return PHPFusion\ImageRepo::getImage($image, $alt, $style, $title, $atts);
	}
	...
	function set_image($name, $path) {
		PHPFusion\ImageRepo::setImage($name, $path);
	}
	...
	function redirect_img_dir($source, $target) {
		PHPFusion\ImageRepo::replaceInAllPath($source, $target);
	}
}
?>