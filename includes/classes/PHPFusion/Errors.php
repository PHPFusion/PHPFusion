...
use PHPFusion\BrowserDetector\Browser;
use PHPFusion\BrowserDetector\Os;
...
class Errors {
	 ...
	 * @return static
	 */
	public static function getInstance(string $key = 'default') {
	...
	public function setError($error_level, $error_message, $error_file, $error_line) {
		...
		if ($db->countRows($result) == 0) {
			...
			$db->query("INSERT INTO ".DB_ERRORS." (
                ...
                :level, :message, :file, :line, :page, :uid, '".$userdata['user_level']."', :user_count, '".USER_IP."', :browser, :os, '".USER_IP_TYPE."', '0', '".time()."'
            )", [
				':level'      => $error_level,
	 ...
	 * @return string
	 */
	public function codeWrap($code, $maxLength = 150) {
		$lines = explode("\n", $code);
		$count = count($lines);
		for ($i = 0; $i < $count; ++$i) {
