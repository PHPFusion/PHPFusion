...
//        $this->input_inline = (!defined('INPUT_INLINE') || INPUT_INLINE);
        $this->info = [
            'section' => $this->registration ? [] : $this->getProfileSections(),
        ...
        $is_core_page = $this->registration || get("section") == 1 || !check_get("section");
        ...
        if ($is_core_page) {
            ...
            if ($this->registration || $this->admin_mode) { // This is registration or within admin.
                // Within admin we need to have to decide whether the user is registered or not.

                if ($this->userData['user_id']) {
                    // this member exists.
                    $this->info['user_password'] .= form_text('user_password1', 'New Password', '', [
                                                                                  'type' => 'password',
                                                                                  'autocomplete_off' => 1,
                                                                                  'inline' => $this->input_inline,
                                                                                  'max_length' => 64,
                    ...
                    // the user has not being registered before.
            ...
            if (!$this->registration) {
                // $this->info['user_admin_password_title'] = form_para($locale['u131'], 'adm_password', 'profile_category_name');
                // $this->info['user_admin_password_sync'] = '';
                // if (defined('ADMIN_PANEL') && iSUPERADMIN) {
                //     //
                //     $this->info['user_admin_password_sync'] = form_checkbox('user_admin_password_sync', 'Set user password as admin password', $this->getInputValue('user_admin_password_sync'));
                // } elseif (iADMIN && !defined('ADMIN_PANEL')) { // user is an administrator
            ...
            if (!$this->registration) {
                if (isset($this->userData['user_avatar']) && $this->userData['user_avatar'] != "") {
                    ...
                    // $locale['u185']
                    $this->info['user_avatar'] = form_fileinput('user_avatar', '', '', [
            ...
            const emailVal = userEmailInput.val();
            userEmailInput.on('change', function(e) {
                if ($(this).val() != emailVal) {
                    $('#user_email_change').show();
                } else {
                   $('#user_email_change').hide();
                }                
            });            
            ");
        ...
        if ($this->registration) {
            $this->info['user_custom'] = (new ProfileRegistrationFields())->render($_POST);
        } else {
            $this->info['user_custom'] = '';
        }
    ...
    private function renderTerms()
    {
        ...
        $modal .= parse_text(\PHPFusion\Quantum\QuantumHelper::parseLabel(fusion_get_settings('license_agreement')));
        $modal_content = '<p class="pull-left">' . $locale['u193a'] . ' ' . ucfirst(showdate('shortdate', fusion_get_settings('license_lastupdate'))) . '</p>';
        $modal_content .= '<button type="button" id="agree" class="btn btn-primary" data-dismiss="modal">' . $locale['u193b'] . '</button>';
    ...
    public function displayProfileOutput()
    {
        ...
        // Add User to Groups
        UserFieldsInput::addCurrentUserToGroup($this->userData['user_id'], $this->userData['user_groups']);
