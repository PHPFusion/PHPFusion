...
class Authenticate {
     ...
     *
     * @param null $authentication_url
     */
    public function __construct($authentication_url = NULL) {

        if ( $authentication_url ) {
            self::$authenticate_url = $authentication_url;
        }

        //return $this->_authenticate($inputUserName, $inputPassword, $remember);
    }
     ...
     */
    public function authenticate($inputUserName, $inputPassword, $remember) {

        $locale = fusion_get_locale('', [ LOCALE . 'English/global.php', LOCALE . 'English/' . 'defender.php' ]);
        $settings = fusion_get_settings();

        $inputUserName = preg_replace([ "/\=/", "/\#/", "/\sOR\s/" ], "", stripinput($inputUserName));

        $where = match ( (int)$settings['login_method'] ) {
            1 => "user_email",
            2 => ( filter_var($inputUserName, FILTER_VALIDATE_EMAIL) ? "user_email" : "user_name" ),
            default => "user_name",
        };

        $result = dbquery("SELECT * FROM " . DB_USERS . " WHERE " . $where . "=:name LIMIT 1",
                          [
                              ':name' => strtolower($inputUserName),
                          ]);
            ...
            if ( $passAuth->isValidCurrentPassword(TRUE) ) {
                ...
                if ( $user['user_status'] == 0 && $user['user_actiontime'] == 0 ) {
                    ...
                    if ( self::$authenticate_url ) {
                        addnotice('success', $locale['global_186'], self::$authenticate_url);
                        redirect(self::$authenticate_url);
                    }
    ...
    public static function storeUserSession(PasswordAuth $passAuth, $user_id) {
        ...
        if ( $passAuth->isValidCurrentPassword(TRUE) ) {
            ...
            dbquery("UPDATE " . DB_USERS . " SET user_session=:s01 WHERE user_id=:u01",
                    [
                        ':u01' => $user_id,
                        ':s01' => $session,
                    ]);
    ...
    public static function setAdminLogin() {
        ...
        if ( check_get("logout") ) {
            unset($_SESSION['admin_aidlink'], $_SESSION['admin_aidlink_context'], $_SESSION['aid']);
        ...
        if ( ! defined('FUSION_API_REQUEST') && check_post("admin_password") ) {

            $admin_password = sanitizer('admin_password');
    ...
    public static function validateAuthAdmin($pass = "") {
        ...
        if ( iADMIN ) {
            ...
            if ( $pass == '' && isset($_COOKIE[ COOKIE_ADMIN ]) && $_COOKIE[ COOKIE_ADMIN ] != "" ) {
                        ...
                        if ( dbrows($result) == 1 ) {
                            ...
                            if ( $cookieHash == $hash ) {
                                ...
                                if ( count($token_data) == 3 ) {
                                    [ $tuser_id, $token_time, $hash ] = $token_data;
                ...
                $result = dbquery("SELECT user_admin_algo, user_admin_salt, user_admin_password
                                FROM " . DB_USERS . " 
                                WHERE user_id='" . $userdata['user_id'] . "' AND user_level < " . USER_LEVEL_MEMBER . " AND  user_status='0' AND user_actiontime='0' LIMIT 1
                                ");
        ...
        if ( isset($_COOKIE[ COOKIE_USER ]) && $_COOKIE[ COOKIE_USER ] != "" ) {
            ...
            if ( count($cookieDataArr) == 3 ) {
                ...
                if ( $cookieExpiration > time() ) {
                    ...
                    $param = [':uid' => isnum($userID) ? $userID : 0 ];
