...
class InstallCore extends Infusions {
    ...
    protected static $installer_mode = self::MODE_INSTALL;
    /*
    ...
    /**
     * Resolve the installer mode once and keep it stable across step posts.
     */
    private static function resolveInstallerMode(): void {
        $requested_mode = isset($_GET['mode']) ? strtolower(trim((string)$_GET['mode'])) : '';
        $allowed_modes = [self::MODE_INSTALL, self::MODE_UPGRADE, self::MODE_RECOVERY];

        if (in_array($requested_mode, $allowed_modes, TRUE)) {
            self::$installer_mode = $requested_mode;
            $_SESSION['installer_mode'] = $requested_mode;
        } else if (!empty($_SESSION['installer_mode']) && in_array($_SESSION['installer_mode'], $allowed_modes, TRUE)) {
            self::$installer_mode = $_SESSION['installer_mode'];
        }
    }

    protected static function setInstallerMode(string $mode): void {
        if (in_array($mode, [self::MODE_INSTALL, self::MODE_UPGRADE, self::MODE_RECOVERY], TRUE)) {
            self::$installer_mode = $mode;
            $_SESSION['installer_mode'] = $mode;
        }
    }

    public static function getInstallerMode(): string {
        return self::$installer_mode;
    }

    public static function isRecoveryMode(): bool {
        return self::$installer_mode === self::MODE_RECOVERY;
    }

    public static function isUpgradeMode(): bool {
        return self::$installer_mode === self::MODE_UPGRADE || isset($_GET['upgrade']);
    }

    protected static function installerUrl(array $parameters = []): string {
        $parameters += [
            'localeset' => LANGUAGE,
            'mode'      => self::$installer_mode,
        ];

        return FUSION_SELF.'?'.http_build_query($parameters);
    }
