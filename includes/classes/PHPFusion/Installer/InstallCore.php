...
class InstallCore extends Infusions
{
	...
	const STEP_SITE_FORM = 5;            //must be between quotation marks because of implicit type conversion
	const STEP_PRIMARY_ADMIN_FORM = 6;   //must be between quotation marks because of implicit type conversion
	const STEP_PRIMARY_ADMIN_SAVE = 6;
	const STEP_LANGUAGE_FORM = 7;
	const STEP_INFUSIONS = 8;
	const STEP_SETUP_COMPLETE = 9;
	const STEP_EXIT = 10;
	const STEP_TRANSFER = 6.1;
	...
	protected static function installerStep($step = 'auto')
	{
		...
		if ($step == 'auto') {
			...
			if (!defined('INSTALLER_STEP')) {
				...
				define('INSTALLER_STEP', $_SESSION['step']);
			}
		} else {
			$_SESSION['step'] = $step;
		}
