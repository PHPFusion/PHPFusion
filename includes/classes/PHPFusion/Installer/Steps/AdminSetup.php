            ...
            if (fusion_safe()) {
                ...
                if (fusion_safe()) {
                    ...
                    if (strpos($enabled_lang, '.')) {
                        ...
                        $result = dbquery("SELECT link_language FROM ".DB_PREFIX."site_links ORDER BY link_language ASC");
                        $installed_languages = [];
                        if (dbrows($result) > 0) {
                        ...
                        $result = dbquery("SELECT admin_language FROM ".DB_PREFIX."admin ORDER BY admin_language ASC");
                        $installed_languages = [];
                        if (dbrows($result) > 0) {
                            while ($data = dbarray($result)) {
                         ...
                         */
                        $result = dbquery("SELECT template_language FROM ".DB_PREFIX."email_templates ORDER BY template_language ASC");
                        $installed_languages = [];
                        if (dbrows($result) > 0) {
                            while ($data = dbarray($result)) {
