...
class AdminSetup extends InstallCore {
    ...
    private function validateSiteData() {
        $enabled_languages = $_POST['enabled_languages'] ?? [];
        if (!is_array($enabled_languages)) {
            $enabled_languages = [];
        }

        // The installer locale checkbox is disabled, so browsers do not submit it.
        // Restore it server-side and accept only detected locale keys.
        $enabled_languages[] = self::$localeset;
        $enabled_languages = array_values(array_unique(array_filter(
            stripinput($enabled_languages),
            static function ($language) {
                return is_string($language) && array_key_exists($language, self::$locale_files);
            }
        )));
