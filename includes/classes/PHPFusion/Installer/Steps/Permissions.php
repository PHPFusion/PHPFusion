        ...
        foreach ($system_requirements as $test) {
			...
			if (!empty($test['sub'])) {
                ...
                foreach ($test['sub'] as $key => $value) {
						...
						if ( !empty( $value[1] ) ) :
                        	$sub_class = 'text-danger';
							$values = '<iconify-icon icon="solar:danger-circle-linear" class="me-1"></iconify-icon>';
							...
							$values = '<iconify-icon icon="solar:check-read-linear" class="me-1"></iconify-icon>';
						endif;

                        $warned_content .= '<tr class="border-bottom border-white border-opacity-10">
                            <td class="py-1 text-muted">'.$key.'</td>
                            <td class="py-1 text-end '.$sub_class.' fw-medium">'.$values.$value[0].'</td>
                        </tr>';
                }
