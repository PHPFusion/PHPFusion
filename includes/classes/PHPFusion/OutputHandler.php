...

{
	 ...
	 * @param string $content
	 */
	public static function setOG($name, $content = "")
	{
		self::$pageOg[$name] = $content;
	}
	...
	public static function addToJQuery($code = "", $order = 0)
	{
		
		//!stristr(self::$jqueryCode[$order], $code)
		
		if (!empty(self::$jqueryCode[$order])) {
			
			self::$jqueryCode[$order] .= $code;
	...
	public static function addToJavascript($code = '')
	{
	...
	public static function handleOutput($output)
	{
		...
		if (!empty(self::$pageOg)) {
			foreach (self::$pageOg as $name => $content) {
				// Regex to check if the meta tag exists
				$pattern = "#<meta (http-equiv|property)=(['\"])og:" . preg_quote($name, '#') . "\\2 content=(['\"]).*?\\3#i";
