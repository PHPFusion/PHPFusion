...
class Comments {
    ...
    private $locale = [];
    private $commentType = [];
    private $allowed_section = ['comments_view', 'comments_edit'];
    ...
    public function display_administration_form() {
        ...
        openside('Comments', $this->getCommentNavs());
        //echo opentab($master_tab_title, $_GET['section'], 'comments_view', TRUE, 'nav-tabs m-b-20');
        switch ($section) {
            case "comments_view":

                //$remote_file = fusion_get_settings('siteurl').'administration/includes/comments.json.php';
                $remote_file = fusion_get_settings('siteurl').'administration/includes/comments.json.php'.fusion_get_aidlink().'&auth='.$_COOKIE[COOKIE_PREFIX.'user'].'&commentType='.fusion_encode($this->commentType);
                $table_id = fusion_table('comments_admin', [
                    'remote_file' => $remote_file,
                    //'ajax_data'   => [
                    //    'commentType' => fusion_encode($this->commentType),
                    //    'auth'        => $_COOKIE[COOKIE_PREFIX.'user'],
                    //],
                ...
                echo "<thead>";
                echo "<tr><th>Author</th><th>Comment</th><th>Comment In</th><th>Submitted On</th></tr>";
                echo "<tbody></tbody>";
                echo "</table>";
    ...
    private function getSection() {
        if ($section = get('section')) {
            if (in_array($section, $this->allowed_section)) {
                return $section;
            }
        }
        return $this->allowed_section[0];
    }

    private function getCommentNavs() {
        $navigation = [];
        $ctype = get('ctype');
        foreach ($this->commentType as $key => $value) {
            $nav_arr = [
                'link'   => FUSION_SELF.$this->aidlink.'&amp;ctype='.$key,
                'title'  => $value,
                'active' => $ctype == $key ? TRUE : FALSE,
            ];
            array_push($navigation, $nav_arr);
        }
        sorter($navigation, 'title');
        return $navigation;
    }
