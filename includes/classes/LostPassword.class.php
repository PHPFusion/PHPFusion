...
class LostPassword extends PasswordAuth {
    ...
    public function sendPasswordRequest($email) {
        ...
        if ($this->_isValidEMailAddress($email)) {
            ...
            $mailBody = str_replace(
                ["[USER_NAME]", "[NEW_PASS_LINK]", '[SITENAME]', '[SITEUSERNAME]'],
                [$data['user_name'], $link, $settings['sitename'], $settings['siteusername']],
                $locale['410']
            );
    ...
    private function _sendNewPassword() {
        ...
        $mailbody = str_replace(
            ["[USER_NAME]", "[NEW_PASS]", "[SITENAME]", "[SITEUSERNAME]"],
            [$this->_userName, $this->_newPassword, $settings['sitename'], $settings['siteusername']],
            $locale['411']
        );
