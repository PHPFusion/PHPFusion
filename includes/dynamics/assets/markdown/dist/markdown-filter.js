...
`;
  ...
  // includes/dynamics/assets/markdown/src/markdown-filter.js
  ...
  var safeInlineTags = ["u", "sub", "sup", "mark"];
  ...
  function preserveSafeInlineTags(source) {
    ...
    const markdownSource = source.replace(tagPattern, (match2, tag, content) => {
  ...
  function restoreSafeInlineTags(html, markers) {
    return markers.reduce((output, marker) => output.split(marker.open).join(`<${marker.tag}>`).split(marker.close).join(`</${marker.tag}>`), html);
  }
