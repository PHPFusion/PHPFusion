...
global $_POST;
    ...
    if (isset($chain_to_parent_id) && ($chain_to_parent_id !="")) { $chain_to_parent_id = stripinput($chain_to_parent_id); } else { $chain_to_parent_id = ""; }
    if (isset($name) && ($name !="")) { $name = stripinput($name); } else { $name = ""; }
    if (isset($self_id) && ($self_id !="")) { $self_id = stripinput($self_id); } else { $self_id = ""; }
    if (isset($self_cat) && ($self_cat !="")) { $self_cat = stripinput($self_cat); } else { $self_cat = ""; }
    if (isset($master_id) && ($master_id !="")) { $master_id = stripinput($master_id); } else { $master_id = ""; }
    ...
    if (!isset($child_level)) { $child_level = 0; } // first run must reset the space.
    ...
    if (is_array($option_array)) {
        ...
        foreach($option_array as $arr) { // outputs: key, value, class - in order
            if ($input_value !== "" && ($input_value == $arr[$self_id])) { $select = "selected"; } else { $select = ""; }
            ...
            if (array_key_exists('children', $arr)){
                if ($arr[$self_cat] == "0") { $indent = 1; } else { $indent = 2; $child_level++; }
    ...
    } else { $html .="<option value=''>No Option Available</option>"; }
...
function form_select_chain($title, $input_name, $input_id, $option_array, $input_value=false, $chain_to_parent_id, $array=false) {
    ...
    //if (isset($input_value) && ($input_value !=="")) { $input_value = stripinput($input_value); } else { $input_value = ""; }
    if (isset($is_order) && ($is_order !=="")) { $is_order = stripinput($is_order); } else { $is_order = ""; }
    ...
    if (is_array($option_array)) {
        foreach($option_array as $arr) { // outputs: key, value, class - in order
            //print_p($arr);
            if ($is_order == 1) {
                if ($input_value !== "" && ($arr['3'] == "1")) { $select = "selected"; } else { $select = ""; }
            } else {
                // $arr['1'] = link_value
                if ($input_value !== "" && ($input_value == $arr['1'])) { $select = "selected"; } else { $select = ""; }
            }
            // the fapi requires = array("1"=>"country", "2"=>"value");
            // $arr['2'] = parent value.
            if (array_key_exists("2", $arr)) { $subclass = "class='".$arr['2']."'"; } else { $subclass = ""; } // this one is used as chain selects
