...
function form_select($title, $input_name, $input_id, array $option_array = array(), $input_value = FALSE, array $options = array()) {
	...
	if ($options['jsonmode'] || $options['tags']) {
		...
		$html .= "<input ".($options['required'] ? "class='req'" : '')." type='hidden' name='$input_name' id='$input_id' ".(($options['width']) ? "style='width: ".$options['width']."'" : "style='min-width: 250px'").">\n";
	} else {
		// normal mode
		$html .= "<select name='$input_name' id='$input_id' ".(($options['width']) ? "style='width: ".$options['width']." " : "style='width:250px; max-width: 250px'").($options['deactivate'] ? 'disabled' : '').($options['multiple'] ? 'multiple' : '')." />";
...
function form_select_tree($title, $input_name, $input_id, $input_value = FALSE, $array = FALSE, $db, $name_col, $id_col, $cat_col, $self_id = FALSE, $id = FALSE, $level = FALSE, $index = FALSE, $data = FALSE) {
	...
	if (!$level) {
		$level = 0;
		$html = "<div id='$input_id-field' class='form-group m-t-10 ".$class."'>\n";
