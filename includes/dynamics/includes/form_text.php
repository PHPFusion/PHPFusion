...
function form_text($title = FALSE, $input_name, $input_id, $input_value = FALSE, $options = array()) {
	...
	$options += array(
		...
		'append_button' => !empty($options['append_button']) ? : '',
		'append_value' => !empty($options['append_value']) ? : '<i class="entypo search"></i>',
		'append_size' => !empty($options['append_size']) ? : '',
		'append_class' => !empty($options['append_class']) ? : 'btn-default',
		'append_type' => !empty($options['append_type']) ? : 'submit',
		'prepend_button' => !empty($options['prepend_button']) ? : '',
		'prepend_value' => !empty($options['prepend_value']) ? : '<i class="entypo search"></i>',
		'prepend_size' => !empty($options['prepend_size']) ? : '',
		'prepend_class' => !empty($options['prepend_class']) ? : 'btn-default',
		'prepend_type' => !empty($options['prepend_class']) ? : 'submit',
		'autocomplete_off' => !empty($options['autocomplete_off']) ? : '0',
		'password' => !empty($options['password']) && $options['password'] ==1 ? : '0',
		'email' => !empty($options['email']) && $options['email'] ==1 ? : '0',
		'number' => !empty($options['number']) && $options['number'] ==1 ? : '0',
		'url' => !empty($options['url']) && $options['url'] ==1 ? : '0',
