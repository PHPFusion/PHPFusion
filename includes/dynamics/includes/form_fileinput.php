...
function form_fileinput($input_name, $label = '', $input_value = FALSE, array $options = []) {
    ...
    $template_choices = array("classic", "modern", "thumbnail", "avatar", "button");
    ...
    $default_options = [
        ...
        "async"                   => "false",
        "dropzone"                => FALSE,
        "valid_ext"               => ".JPG,.JPEG,.BMP,.GIF,.TIFF,.PNG,.WEBP,.TIF,.TIFF,.SVG",
        "thumbnail"               => FALSE,
        "thumbnail_w"             => 300,
        "thumbnail_h"             => 300,
        "thumbnail_folder"        => "",
        ...
        "croppie_zoom"            => FALSE,
        "croppie_zoomer"          => FALSE,
        "croppie_viewport_width"  => 200, // 200px default
        "croppie_viewport_height" => 200,
    ...
    if ($options['croppie'] === TRUE) {
        ...
        $crop_options = [
            ...
            'enableExif'   => TRUE,
            'showZoomer'   => ($options['croppie_zoomer'] ? TRUE : FALSE),
            'enableZoom'   => ($options['croppie_zoom'] ? TRUE : FALSE),
        ];
        $crop_viewport = [];
        $crop_boundary = [];
        if ($options['croppie_viewport_width'] && $options['croppie_viewport_height']) {
            $crop_viewport = [
                'viewport' => [
                    'width'  => $options['croppie_viewport_width'],
                    'height' => $options['croppie_viewport_height']
                ]
            ];
        }
    ...
    $html .= "<div id='".$options['input_id']."-help' class='error-container'>";
    $html .= (\Defender::inputHasError($input_name)) ? "<div class='label label-danger p-5 display-inline-block'>".$options['error_text']."</div>" : '';
    $html .= "</div>";
    ...
    /** for fileinput, the input_name must be designed for 1 dom only. you cannot have multiple dynamics fulling under the same input_name multiple. */
    if ($options["multiple"]) {
    ...
    if ($options['krajee_disabled'] === FALSE) {
        $valid_ext_js = json_encode(explode(",", $options["valid_ext"]));
        ...
        if ($options['form_id'] && $options['jsonurl']) {
            ...
            $error_event = ($options["async"] === "true") ? "fileuploaderror" : "filebatchuploaderror";
            ...
            $success_js = "
            .on('$success_event', function(event, data, previewId, index) {
            ...
            $change_js = ".on('$change_event', function(event) {
                ".($options["js_change_handler"] ? $options["js_change_handler"]."(event)" : "
        ...
        $script = "let ".$options["input_id"]." = $('#".$options['input_id']."');";
        ...
        switch ($options['template']) {
            ...
            case 'button':
                ...
                ".$options["input_id"].".fileinput({
                    ...
                    captionClass : '',
                    maxFileCount: '".$options['max_count']."',
                    removeClass : 'btn button-default',
                    browseLabel: '$browseLabel',
                    ...
                    $extra_data_js
                    layoutTemplates: {
                        main2: '<div class=\"buttons-wrapper\">'+' {browse}'+'</div>{preview}',
                    },
                    $lang
                })".$change_js.$error_js.$success_js.";";
            ...
            case "modern":
                $script .= "
                   ".$options["input_id"].".fileinput({
                        ...
                        ".($value ? "initialPreview: ".$value.", " : '')."
                        initialPreviewAsData: true,
                        allowedFileExtensions: $valid_ext_js,
                        ".($options['preview_off'] ? "showPreview: false, " : '')."
