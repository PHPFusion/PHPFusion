 ...
 * @return string
 */
function render_textarea_input($input_name, $input_value = '', array $options = []): string
{
    // Determine if it should render the live editor (toolbar) layout
    $is_live = !empty($options['html']) ||
        (!empty($options['type']) && in_array($options['type'], ['html', 'bbcode'])) ||
        !empty($options['bbcode']);
 ...
 * @return string
 */
function live_textarea($input_name, $input_value = '', array $options = []): string
{
 ...
 * @return string
 */
function basic_textarea($input_name, $input_value = '', array $options = []): string
{
    $input_id = $options['input_id'] ?? uniqid('textarea_');
