 ...
 * @return string
 */
function render_collapse( $options ) {

    if ($options['callback'] == 'opencollapse') {

        return '<div class="accordion' . whitespace( $options['class'] ) . '" id="' . $options['id'] . 'CollapseGroup" role="tablist" aria-multiselectable="true">';
        ...
        return
            ...
            . '</h4>'
            . '<div id="' . $options['id'] . 'Collapse" class="accordion-collapse collapse' . ($options['active'] ? ' show' : '') . '" ' . ($options['group_id'] ? 'data-bs-parent="#' . $options['group_id'] . '-CollapseGroup"' : '') . '>'
