            ...
            if (dbrows($result)) {
                ...
                if ($to != $from) {
                    if ($data['user_id'] == 1 || $data['user_level'] > 101 || $msg_settings['pm_inbox'] == "0" || ($data['message_count'] + 1) <= $msg_settings['pm_inbox']) {
                        dbquery("INSERT INTO ".DB_MESSAGES." (message_to, message_from, message_subject, message_message, message_smileys, message_read, message_datestamp, message_folder) VALUES('".$data['user_id']."','".$userdata['user_id']."','".$subject."','".$message."','".$smileys."','0','".time()."','0')");
...
function upload_file(
    ...
    if (is_uploaded_file($_FILES[$source_file]['tmp_name'])) {
        ...
        if ($file['size'] > $max_size) {
            ...
            $upload_file['error'] = 1;
        } else if (!in_array($file_ext, $valid_ext)) {
    ...
    if (is_uploaded_file($_FILES[$source_image]['tmp_name'])) {
        ...
        if (filesize($image['tmp_name']) > 10 && @getimagesize($image['tmp_name'])) {
            ...
            if ($image['size'] > $max_size) {
                // Invalid file size
                $image_info['error'] = 1;
            } else if (!$filetype || !verify_image($image['tmp_name'])) {
