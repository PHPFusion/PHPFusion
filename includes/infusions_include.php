...
if (!function_exists('upload_file')) {
    ...
    function upload_file($source_file, $target_file = "", $target_folder = DOWNLOADS, $valid_ext = ".zip,.rar,.tar,.bz2,.7z", $max_size = 15000, $query = "", $replace_upload = FALSE)
    {
        ...
        if (is_uploaded_file($_FILES[$source_file]['tmp_name'])) {
            ...
            $valid_ext = preg_split("/[,\|]/", $valid_ext);
            $valid_ext = (array)$valid_ext;
...
if (!function_exists('upload_image')) {
     ...
     * @param false $delete_original Set this to true to delete the original image if thumbnail is being created
     ...
     * @param false $replace_upload Replace image if exists in the target folder, true by default
     *
     * @return array Array with information about the upload.
     */
    function upload_image($source_image, $target_name = "", $target_folder = IMAGES, $target_width = 1800, $target_height = 1600, $max_size = 150000, $delete_original = FALSE, $thumb1 = TRUE, $thumb2 = TRUE, $thumb1_ratio = 0, $thumb1_folder = IMAGES, $thumb1_suffix = "_t1", $thumb1_width = 100, $thumb1_height = 100, $thumb2_ratio = 0, $thumb2_folder = IMAGES, $thumb2_suffix = "_t2", $thumb2_width = 400, $thumb2_height = 300, $query = "", array $allowed_extensions = ['.jpg', '.jpeg', '.png', '.png', '.svg', '.gif', '.bmp', '.webp', '.avif'], $replace_upload = true)
    {
...
require_once __DIR__.'/datatable_include.php';

/**
 * Load login
 */
function load_login_connectors() {
    $login_connector_dir = INCLUDES.'login_connectors/';
    if ($files = makefilelist($login_connector_dir)) {
        foreach($files as $file) {
            require_once $login_connector_dir.$file;
        }
    }
}
