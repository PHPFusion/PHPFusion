...
class GeSHi {
    ...
    var $lexic_permissions = array(
        'KEYWORDS' => array(), 'COMMENTS' => array('MULTI' => TRUE), 'REGEXPS' => array(),
    ...
    function set_header_type($type) {
        ...
        if (!in_array($type, array(
            GESHI_HEADER_NONE, GESHI_HEADER_DIV, GESHI_HEADER_PRE, GESHI_HEADER_PRE_VALID,
    ...
    function get_language_name_from_extension($extension, $lookup = array()) {
        if (!is_array($lookup) || empty($lookup)) {
            $lookup = array(
                '6502acme' => array('a', 's', 'asm', 'inc'), '6502tasm' => array('a', 's', 'asm', 'inc'),
                    ...
                    if ($char > 0) {
                        ...
                        if ($char == strlen($prev_keys[$level])) {
                            ...
                            if (in_array($new_key_part1[0], $regex_chars) || in_array($new_key_part2[0], $regex_chars)) {
                                ...
                                // recreate key index
                                array_splice($prev_keys, $level, count($prev_keys), array(
                                    $new_key_part1,
                                    $new_key_part2
                                ));
    ...
    function highlight_lines_extra($lines, $style = NULL) {
        if (is_array($lines)) {
            ...
            if ($style === NULL) { //Check if we should use default style
                ...
                if ($style === FALSE) { //Check if to remove this line
            ...
            while ($i < $length) {
                    ...
                    if (is_array($delimiters)) {
                        ...
                        if (!GESHI_PHP_PRE_433 && //Needs proper rewrite to work with PHP >=4.3.0; 4.3.3 is guaranteed to work.
                            ...
                            if (isset($matches_rx['start']) && isset($matches_rx['end'])) {
                                $matches[$dk] = array(
                                    'next_match' => $matches_rx['start'][1], 'dk' => $dk,
                                    'close_strlen' => strlen($matches_rx['end'][0]),
                                    'close_pos' => $matches_rx['end'][1],
                                );
                            } else {
                                $matches[$dk] = array(
                                    'next_match' => $matches_rx[1][1], 'dk' => $dk,
                ...
                if (is_array($delim_copy[$next_match_pointer['dk']])) {
                    ...
                    while (TRUE) {
                        ...
                        if ($code[$i] == $next_match_pointer['open'][0] && ($next_match_pointer['open_strlen'] == 1 || substr($code, $i,
                                                                                                                              $next_match_pointer['open_strlen']) == $next_match_pointer['open'])
                        ) {
            ...
            if ($highlight_part) {
                ...
                for ($i = 0; $i < $length; ++$i) {
                        ...
                        foreach ($this->language_data['COMMENT_REGEXP'] as $comment_key => $regexp) {
                            ...
                            if (isset($comment_regexp_cache_per_key[$comment_key]) && ($comment_regexp_cache_per_key[$comment_key]['pos'] >= $i || $comment_regexp_cache_per_key[$comment_key]['pos'] === FALSE)) {
                                ...
                                if (//This is to allow use of the offset parameter in preg_match and stay as compatible with older PHP versions as possible
                                    (GESHI_PHP_PRE_433 && preg_match($regexp, substr($part, $i), $match,
                                                                     PREG_OFFSET_CAPTURE)) || (!GESHI_PHP_PRE_433 && preg_match($regexp, $part,
                                                                                                                                $match,
                                                                                                                                PREG_OFFSET_CAPTURE,
                                                                                                                                $i))
                                ) {
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos)) {
                                ...
                                if (isset($this->language_data['ESCAPE_REGEXP']) && $next_escape_regexp_pos < $start) {
                                    ...
                                    foreach ($this->language_data['ESCAPE_REGEXP'] as $escape_key => $regexp) {
                                        ...
                                        if (isset($escape_regexp_cache_per_key[$escape_key]) && ($escape_regexp_cache_per_key[$escape_key]['pos'] >= $start || $escape_regexp_cache_per_key[$escape_key]['pos'] === FALSE)) {
                                            ...
                                            if (//This is to allow use of the offset parameter in preg_match and stay as compatible with older PHP versions as possible
                                                (GESHI_PHP_PRE_433 && preg_match($regexp, substr($part, $start), $match,
                                                                                 PREG_OFFSET_CAPTURE)) || (!GESHI_PHP_PRE_433 && preg_match($regexp,
                                                                                                                                            $part,
                                                                                                                                            $match,
                                                                                                                                            PREG_OFFSET_CAPTURE,
                                                                                                                                            $start))
                                    ...
                                    if (ord($es_char) >= 128) {
                                        ...
                                        if (function_exists('mb_substr')) {
                                            ...
                                            if (!GESHI_PHP_PRE_433 && 'utf-8' == $this->encoding) {
                                                if (preg_match("/[\xC2-\xDF][\x80-\xBF]"."|\xE0[\xA0-\xBF][\x80-\xBF]"."|[\xE1-\xEC\xEE\xEF][\x80-\xBF]{2}"."|\xED[\x80-\x9F][\x80-\xBF]"."|\xF0[\x90-\xBF][\x80-\xBF]{2}"."|[\xF1-\xF3][\x80-\xBF]{3}"."|\xF4[\x80-\x8F][\x80-\xBF]{2}/s",
                        ...
                        if ($this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char && substr($part, $i,
                                                                                                    $hq_strlen) == $hq && ($i != $next_comment_regexp_pos)
                        ) {
                            ...
                            if (!$COMMENT_MATCHED) {
                                    ...
                                    foreach ($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark) {
                                        ...
                                        if (isset($comment_single_cache_per_key[$comment_key]) && ($comment_single_cache_per_key[$comment_key] >= $i || $comment_single_cache_per_key[$comment_key] === FALSE)) {
                                            ...
                                            if (// case sensitive comments
                                                ($this->language_data['CASE_SENSITIVE'][GESHI_COMMENTS] && ($match_i = stripos($part, $comment_mark,
                                                                                                                               $i)) !== FALSE) || // non case sensitive
                                                (!$this->language_data['CASE_SENSITIVE'][GESHI_COMMENTS] && (($match_i = strpos($part, $comment_mark,
                                                                                                                                $i)) !== FALSE))
                                            ) {
                                ...
                                if ($next_comment_single_pos == $i) {
                                    ...
                                    if ((empty($sc_disallowed_before) || ($i == 0) || (FALSE === strpos($sc_disallowed_before,
                                                                                                        $part[$i - 1]))) && (empty($sc_disallowed_after) || ($length <= $i + $com_len) || (FALSE === strpos($sc_disallowed_after,
                                                                                                                                                                                                            $part[$i + $com_len])))
                                    ) {
    ...
    function parse_non_string_part($stuff_to_parse) {
        ...
        foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
            if (!isset($this->lexic_permissions['KEYWORDS'][$k]) || $this->lexic_permissions['KEYWORDS'][$k]) {
                ...
                for ($set = 0, $set_length = count($this->language_data['CACHED_KEYWORD_LISTS'][$k]); $set < $set_length; ++$set) {
                    ...
                    $stuff_to_parse = preg_replace_callback("/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php|aspx?))$disallowed_after_local/$modifiers",
                                                            array(
                                                                $this,
            ...
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (is_array($regexp)) {
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        ...
                        $stuff_to_parse = preg_replace_callback("/".$regexp[GESHI_SEARCH]."/{$regexp[GESHI_MODIFIERS]}", array(
                            $this,
                        ...
                        $stuff_to_parse = preg_replace('/'.$regexp[GESHI_SEARCH].'/'.$regexp[GESHI_MODIFIERS],
                    ...
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        // produce valid HTML when we match multiple lines
                        $this->_hmr_key = $key;
                        $stuff_to_parse = preg_replace_callback("/(".$regexp.")/", array(
                            $this,
                            'handle_multiline_regexps'
                        ), $stuff_to_parse);
                        $this->_hmr_key = '';
        ...
        if ($this->lexic_permissions['METHODS'] && $this->language_data['OOLANG']) {
            ...
            foreach ($this->language_data['OBJECT_SPLITTERS'] as $key => $splitter) {
                if (FALSE !== strpos($stuff_to_parse, $splitter)) {
                    ...
                    $stuff_to_parse = preg_replace("/($oolang_before)(".preg_quote($this->language_data['OBJECT_SPLITTERS'][$key],
        ...
        //
        if ($this->lexic_permissions['BRACKETS']) {
            $stuff_to_parse = str_replace($this->language_data['CACHE_BRACKET_MATCH'], $this->language_data['CACHE_BRACKET_REPLACE'],
                                          $stuff_to_parse);
        }
        ...
        if ($this->lexic_permissions['SYMBOLS'] && !empty($this->language_data['SYMBOLS'])) {
            ...
            $n_symbols = preg_match_all("/<\|(?:<DOT>|[^>])+>(?:(?!\|>).*?)\|>|<\/a>|(?:".$this->language_data['SYMBOL_SEARCH'].")+(?![^<]+?>)/",
        ...
        foreach (array_keys($this->language_data['REGEXPS']) as $key) {
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (is_callable($this->language_data['STYLES']['REGEXPS'][$key])) {
                    $this->_rx_key = $key;
                    $stuff_to_parse = preg_replace_callback("/!REG3XP$key!(.*)\|>/U", array(
                        $this,
