...
for ($i = 0; $i < count($matches[1]); $i++) {
    ...
    if (count($lines) < 200) {
        ...
        for ($i = 0; $i < $ccount; $i++) {
            ...
            $text = preg_replace("#\[geshi=(.*?)\](.*?)\[/geshi\]#sie",
                                 "'<div class=\'tbl-border tbl2\' style=\'width:400px\'>".$geshi_save."<strong><i><u>".$locale['bb_geshi_parser1'].":</u></i> ".$locale['bb_geshi_parser2'].":</strong></div><div class=\'tbl-border tbl1\' style=\'width:400px;white-space:nowrap;overflow:auto\'><code style=\'white-space:nowrap\'>'.formatcode('\\2').'<br /><br /><br /></code></div>'",
                                 $text);
        }
    }
    unset($lines);
