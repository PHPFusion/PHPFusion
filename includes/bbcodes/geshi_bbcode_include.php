...
for ($i = 0; $i < count($matches[1]); $i++) {
    ...
    if (count($lines) < 200) {
        ...
        $text2 = $geshi_save."<strong>GeSHi: ".$geshi->get_language_name()."</strong><code>".$geshi->parse_code()."</code>";
        ...
        for ($i = 0; $i < $ccount; $i++) {
            ...
            $text = preg_replace("#\[geshi=(.*?)\](.*?)\[/geshi\]#sie",
                                 $geshi_save."<strong><i><u>".$locale['bb_geshi_parser1'].":</u></i> ".$locale['bb_geshi_parser2'].":</strong><code style=\'white-space:nowrap\'>'.formatcode('\\2').'<br /></code>'",
                                 $text);
        }
    }
    unset($lines);
