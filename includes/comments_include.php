...
function showcomments($ctype, $cdb, $ccol, $cid, $clink) {
    ...
    $comment_name = '';
    $link = '';
    $clink = FUSION_REQUEST;
    $clink = preg_replace("^(&amp;|\?)c_action=(edit|delete)&amp;comment_id=\d*^", "", $clink);
    ...
    if (iMEMBER && (isset($_GET['c_action']) && $_GET['c_action'] == "delete") && (isset($_GET['comment_id']) && isnum($_GET['comment_id']))) {
        if ((iADMIN && checkrights("C")) || (iMEMBER && dbcount("(comment_id)", DB_COMMENTS, "comment_id='".$_GET['comment_id']."' AND comment_name='".$userdata['user_id']."'"))) {
            dbquery("DELETE FROM ".DB_COMMENTS."
    ...
    if ($settings['comments_enabled'] == "1") {
        if ((iMEMBER || $settings['guestposts'] == "1") && isset($_POST['post_comment'])) {
            ...
            if (iMEMBER && (isset($_GET['c_action']) && $_GET['c_action'] == "edit") && (isset($_GET['comment_id']) && isnum($_GET['comment_id']))) {
                ...
                if ((iADMIN && checkrights("C")) || (iMEMBER && dbcount("(comment_id)", DB_COMMENTS, "comment_id='".$_GET['comment_id']."' AND comment_item_id='".$cid."'
                    ...
                    if ($comment_message) {
                        dbquery("UPDATE ".DB_COMMENTS." SET comment_message='".$comment_message."'
                                            WHERE comment_id='".$_GET['comment_id']."'".(iADMIN ? "" : "
                                            AND comment_name='".$userdata['user_id']."'"));
                ...
                if ($comment_name && $comment_message) {
                    ...
                    if (!flood_control("comment_datestamp", DB_COMMENTS, "comment_ip='".USER_IP."'")) {
                        dbquery("INSERT INTO ".DB_COMMENTS." (
        ...
        $c_arr = ["c_con" => [], "c_info" => ["c_makepagenav" => FALSE, "admin_link" => FALSE]];
        ...
        if (dbrows($result)) {
            ...
            while ($data = dbarray($result)) {
                ...
                if ((iADMIN && checkrights("C")) || (iMEMBER && $data['comment_name'] == $userdata['user_id'] && isset($data['user_name']))) {
                    ...
                    $c_arr['c_con'][$i]['edit_dell'] .= "<a href='".($settings['site_seo'] ? PERMALINK_CURRENT_PATH : FUSION_REQUEST)."".($settings['site_seo'] ? '?' : '&amp;')."c_action=edit&amp;comment_id=".$data['comment_id']."#edit_comment'>";
                    $c_arr['c_con'][$i]['edit_dell'] .= $locale['c108']."</a> |\n";
                    $c_arr['c_con'][$i]['edit_dell'] .= "<a href='".($settings['site_seo'] ? PERMALINK_CURRENT_PATH : FUSION_REQUEST)."".($settings['site_seo'] ? '?' : '&amp;')."c_action=delete&amp;comment_id=".$data['comment_id']."' onclick=\"return confirm('".$locale['c110']."');\">";
            ...
            if (iADMIN && checkrights("C")) {
                ...
                $c_arr['c_info']['admin_link'] .= "<a href='".ADMIN."comments.php".$aidlink."&amp;ctype=".$ctype."&amp;comment_item_id=".$cid."'>".$locale['c106']."</a>";
            }
        }
        ...
        if (iMEMBER || $settings['guestposts'] == "1") {
            ...
            echo "<form name='inputform' method='post' action='".($settings['site_seo'] ? BASEDIR : '').$clink."'>\n";
            echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>";
            if (iGUEST) {
                echo "<tr><td class='tbl'>\n".$locale['c104']."</td></tr>\n";
                echo "<tr><td class='tbl'><input type='text' name='comment_name' maxlength='30' class='textbox' style='width:360px' /></td></tr>\n";
            }
            echo "<tr><td class='tbl1'><textarea name='comment_message' cols='70' rows='6' class='textbox p-15' style='width:100%'>".$comment_message."</textarea></td></tr>\n";
            if (iGUEST && (!isset($_CAPTCHA_HIDE_INPUT) || (isset($_CAPTCHA_HIDE_INPUT) && !$_CAPTCHA_HIDE_INPUT))) {
                $_CAPTCHA_HIDE_INPUT = FALSE;
                echo "<div style='width:360px; margin:15px auto;'>";
            ...
            echo "</td></tr>\n";
            echo "<tr><td class='tbl2 text-center'><input type='submit' align='center' name='post_comment' value='".($comment_message ? $locale['c103'] : $locale['c102'])."' class='button' /></td></tr>";
            echo "</table>\n</form>\n";
        } else {
            echo "<div class='well'>\n";
            echo $locale['c105']."\n";
