...
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "1") { $filter = "iorder ASC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "2") { $filter = "sellcount DESC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "3") { $filter = "id DESC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "4") { $filter = "price ASC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "5") { $filter = "price DESC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "6") { $filter = "title ASC"; }
if (isset($_COOKIE['Filter']) && $_COOKIE['Filter'] == "7") { $filter = "title DESC"; }
...
if (!isset($_POST['agreement']) || !isnum($_POST['agreement'])) { die("Denied"); exit; }

if (!isset($_POST['paymethod']) || !isnum($_POST['paymethod'])) { die("Denied"); exit; }
if (!isset($_POST['shipping']) || !isnum($_POST['shipping'])) { die("Denied"); exit; }

if (isset($_POST['shipping'])) { $shipmethod = stripinput($_POST['shipping']); }
if (isset($_POST['paymethod'])) { $paymethod = stripinput($_POST['paymethod']); }

if (isset($_POST['cupon'])) { if (!preg_match("/^[\w\W-0-9A-Z@\s]+$/i", $_POST['cupon'])) { die("Denied"); exit; }  }
...
if (isset($_POST['message'])) { $message = stripinput($_POST['message']); }
...
$shipping = dbarray(dbquery("SELECT * FROM ".DB_ESHOP_SHIPPINGITEMS." WHERE active='1' AND sid='".$shipmethod."' ORDER BY cid,sid ASC"));
    $destlocale= "";
    if ($shipping['destination'] == "1") { $destlocale = $locale['D101']; }
	if ($shipping['destination'] == "2") { $destlocale = $locale['D102']; }
	if ($shipping['destination'] == "3") { $destlocale = $locale['D103']; }
    $order.="<tr>
...
if (!dbrows($verifycupon) != 0) {
	$cupon = dbarray(dbquery("SELECT * FROM ".DB_ESHOP_CUPONS." WHERE cuid='".$cupons."' AND active = '1' AND (custart='0'||custart<=".time().") AND (cuend='0'||cuend>=".time().") LIMIT 0,1"));
...
if ($data['cdynt'] || $data['cdyn']) { $order.="".$data['cdynt']." : ".$data['cdyn'].""; } 
